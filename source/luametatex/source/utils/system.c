...
{
    main_state.starttime = get_current_time();
}

double get_run_time(void)
{
    return get_current_time() - main_state.starttime;
}
...
/*tex
    ...
    \startitemize[n]
        \startitem
            ...
            influence the \PDF\ timestamp and \PDF\ id that is derived from the
            time. This variable is consulted when the kpse library is enabled
            which is analogue to other properties.
        \stopitem
        \startitem
            By setting the |texconfig.start_time| variable (as with other variables
            we use the internal name there). This has the same effect as (1) and
            is provided for when kpse is not used to set these variables or when
            an overloaded is wanted. This is analogue to other properties.
        \stopitem
    \stopitemize
...
/*tex
    ...
    All our interrupt handler has to do is set \TEX's global variable
    |interrupt|; then they will do everything needed.
...
void get_date_and_time(int *minutes, int *day, int *month, int *year, int *utc)
{
