...
static int scan_math(halfword, int);
static int scan_math_style(halfword, int);

static halfword fin_mlist(halfword);

/*tex

    When \TEX\ reads a formula that is enclosed between |$|'s, it constructs an
    \quote {mlist}, which is essentially a tree structure representing that
    formula. An mlist is a linear sequence of items, but we can regard it as a
    tree structure because mlists can appear within mlists. For example, many of
    the entries can be subscripted or superscripted, and such \quote {scripts}
    ...
    until the formula has been fully scanned. For example, when the formula |$a+b
    \over c+d$| is being read, there is no way to tell that |a+b| will be in
    script size until |\over| has appeared.
    ...
    a construct like |\sqrt| that must be built up. This classification appears
    in the mlist data structure.
    ...
    styles by a \quote {top-down} process starting at the outermost level and working
    ...
    \quote {noads} (pronounced \quotation {no-adds}), to distinguish them from
    the somewhat similar \quote {nodes} in hlists and vlists. Certain kinds of
    ordinary nodes are allowed to appear in mlists together with the noads; \TEX\
    tells the difference by means of the |type| field, since a noad's |type| is
    always greater than that of a node. An mlist does not contain character
    nodes, hlist nodes, vlist nodes, math nodes or unset nodes; in particular,
    each mlist item appears in the variable-size part of |mem|, so the |type|
    ...
    \startitemize
        ...
        \startitem
            ...
            the same as |q=null|; for example, |$P_{\}$| and |$P$| produce
            different results (the former will not have the \quote {italic
            correction} added to the width of |P|, but the \quote {script skip}
            will be added).
        \stopitem
...
static void unsave_math(void)
{
    ...
    if (saved_type(0) != saved_textdir) {
        confusion("unsave_math");
    }
...
static void unsave_math_fam_data(int gl)
{
    ...
    if (math_fam_head->stack == NULL)
        return;
    while (math_fam_head->sa_stack_ptr > 0 &&
           abs(math_fam_head->stack[math_fam_head->sa_stack_ptr].level)
           >= (int) gl) {
        st = math_fam_head->stack[math_fam_head->sa_stack_ptr];
        if (st.level > 0) {
...
static void unsave_math_param_data(int gl)
{
    ...
    if (math_param_head->stack == NULL)
        return;
    while (math_param_head->sa_stack_ptr > 0 &&
           abs(math_param_head->stack[math_param_head->sa_stack_ptr].level)
           >= (int) gl) {
        st = math_param_head->stack[math_param_head->sa_stack_ptr];
        if (st.level > 0) {
...
/*tex
    ...
    following; and a \quote {thick space} will be inserted just before the formula
    ...
    by giving the\quote {small} and \quote {large} starting characters, as
    explained in Chapter~17 of {\em The \TEX book}.
    ...
    thick to make a fraction rule; however, the special value
    |preset_rule_thickness| is used to stand for the |preset_rule_thickness| of
    the current size. The |numerator| and |denominator| point to mlists that
    define a fraction; we always have |type(numerator) = type(denominator) =
    sub_mlist|. The |left_delimiter| and |right_delimiter| fields specify
    delimiters that will be placed at the left and right of the fraction. In this
    way, a |fraction_noad| is able to represent all of \TEX's operators |\over|,
...
halfword new_noad(void)
{
    ...
    return new_node(simple_noad, ord_noad_type);
}

halfword new_sub_box(halfword curbox)
{
    halfword p = new_noad();
    nucleus(p) = new_node(sub_box_node, 0);
...
/*tex
    ...
    And finally, we have the |fence_noad| type, to implement \TEX's |\left| and
    |\right| as well as \ETEX's |\middle|. The |nucleus| of such noads is
    replaced by a |delimiter| field; thus, for example, |\left(| produces a
    |fence_noad| such that |delimiter(p)| holds the family and character codes
    ...
    |display_style|, \dots, |script_script_style|, and adds~1 to get the \quote
    {cramped} versions of these styles. This gives a numerical order that is
    backwards from the convention of Appendix~G in {\em The \TEX book}; i.e., a
...
static halfword new_choice(void)
{
...
/*tex
    ...
    in the (ridiculous) formula {$\sqrt {a ^ {\mathinner {b _ {c \over x+y}}}}$|.

*/

static void display_normal_noad(halfword p);
static void display_fence_noad(halfword p);
static void display_fraction_noad(halfword p);

void show_math_node(halfword p)
{
    switch (type(p)) {
        case style_node:
...
static void print_subsidiary_data(halfword p, unsigned char c)
{
...
static void display_fraction_noad(halfword p)
{
    ...
    if (thickness(p) == preset_rule_thickness)
        tprint("= default");
    else
...
/*tex
    ...
    make \quote {noads} as well as nodes, so the reader should review the discussion
...
static void new_save_level_math(group_code c)
{
    ...
    text_dir_ptr = new_dir(normal_dir,math_direction_par);
    incr(save_ptr);
    new_save_level(c);
...
/*tex
    ...
    We now just return the direction.

*/

static int math_and_text_reversed_p(void)
{
    int i = save_ptr - 1;
    while (save_type(i) != level_boundary)
        i--;
    while (i < save_ptr) {
        if (save_type(i) == restore_old_value && save_value(i) == int_base + par_direction_code) {
            /*
...
static void enter_display_math(void)
{
    ...
    */
    if (head == tail ||
        ...
        line_break(1, math_shift_group);
        w = actual_box_width(just_box, x_over_n(quad(cur_font_par),1000) * math_pre_display_gap_factor_par);
    }
    /*tex
    ...
    */
    ...
    /*tex so -1 meant opposite, now we return the real value */
 /* eq_word_define(int_base + pre_display_direction_code, (math_and_text_reversed_p() ? -1 : 0)); */
    eq_word_define(int_base + pre_display_direction_code, math_and_text_reversed_p());
...
/*tex
    ...
    tells what syntax form to use (\TEX\ or \LUATEX) , the |doclass| tells
    whether or not read a math class also (for |\delimiter| c.s.). (the class is
    passed on for conversion to |\mathchar|).

*/

static delcodeval do_scan_extdef_del_code(int extcode, int doclass)
{
    delcodeval d;
...
void scan_extdef_del_code(int level, int extcode)
{
    ...
    d = do_scan_extdef_del_code(extcode, 0);
...
mathcodeval scan_mathchar(int extcode)
{
    ...
    if (extcode == tex_mathcode) {
        ...
        if (cur_val > 0x8000) {
            ...
            if (mchr > 0x10FFFF) {
                tex_error(
                    "Invalid math code during > 0x8000 mathcode fallback",
                    "I'm going to use 0 instead of that illegal code value."
                );
                mcls = 0;
                mfam = 0;
...
mathcodeval scan_delimiter_as_mathchar(int extcode)
{
    delcodeval dval;
    mathcodeval mval;
    dval = do_scan_extdef_del_code(extcode, 1);
...
static int scan_math_style(halfword p, int mstyle)
{
    get_next_nb_nr();
    back_input();
    scan_left_brace();
...
static int scan_math(halfword p, int mstyle)
{
...
void set_math_char(mathcodeval mval)
{
    if (mval.class_value == 8) {
        ...
        halfword p = new_noad();
        halfword q = new_node(math_char_node, 0);
        nucleus(p) = q;
...
void math_char_in_text(mathcodeval mval)
{
    if (mval.class_value == 8) {
        ...
        halfword p = new_char(fam_fnt(mval.family_value, text_size), mval.character_value, 0); /* todo: data */
        vlink(tail) = p;
        tail = p;
    }
}
...
static void scan_delimiter(halfword p, int r)
{
    delcodeval dval = { 0, 0, 0, 0, 0 };
    if (r == tex_mathcode) {
        /*tex |\radical| */
        dval = do_scan_extdef_del_code(tex_mathcode, 1);
    } else if (r == umath_mathcode) {
        /*tex |\Uradical| */
        dval = do_scan_extdef_del_code(umath_mathcode, 0);
        ...
        switch (cur_cmd) {
            ...
            case delim_num_cmd:
                if (cur_chr == 0) {
                    ...
                    /*tex |\Udelimiter| */
                    dval = do_scan_extdef_del_code(umath_mathcode, 1);
                } else {
...
halfword math_vcenter_group(halfword p)
{
    halfword r;
    halfword q = new_noad();
...
{
    halfword p;
    int prev_style = m_style;
    unsave_math();
    p = fin_mlist(null);
    if (saved_type(-1) != saved_choices) {
        confusion("build_choices");
    }
...
static void do_sub_sup(int no)
{
    ...
    if (cur_cmd == sup_mark_cmd || cur_chr == sup_mark_cmd) {
        ...
        q = new_node(math_char_node, 0);
        supscr(tail) = q;
        (void) scan_math(q, sup_style(m_style));
        ...
        q = new_node(math_char_node, 0);
        subscr(tail) = q;
        (void) scan_math(q, sub_style(m_style));
    }
}
...
void math_fraction(void)
{
    ...
    if (incompleat_noad_par != null) {
        ...
        halfword temp_value = new_node(sub_mlist_node, 0);
        ...
        if ((c % delimited_code) == skewed_code) {
            halfword q = new_node(delim_node, 0);
        ...
        switch (c % delimited_code) {
            ...
            case over_code:
                {
                    thickness(incompleat_noad_par) = preset_rule_thickness;
                }
                break;
            case atop_code:
                {
...
/*tex
    ...
    upon to return a halfword to the newly completed mlist, and to pop the nest
...
static halfword fin_mlist(halfword p)
{
    halfword q;
    if (incompleat_noad_par != null) {
...
/*tex
    ...
    removed when they surround a single Ord without sub- and/or superscripts, or
    when they surround an accent that is the nucleus of an Ord atom.

*/

void close_math_group(halfword p)
{
...
void math_left_right(void)
{
    ...
    if ((t != no_noad_side) && (t != left_noad_side) && (cur_group != math_left_group)) {
        ...
        halfword q; /*tex resulting mlist */
        halfword p = new_noad();
        halfword r = new_node(delim_node, 0);
        type(p) = fence_noad;
...
static void check_inline_math_end(void)
{
    if (cur_chr != cramped_text_style) {
        tex_error(
            "Inline math should end with \\Ustopmath",
            "I shall assume that you typed that."
        );
    }
}
...
static void finish_displayed_math(int l, halfword eqno_box, halfword p)
{
    ...
    int swap_dir = (pre_display_direction_par < 0 ? 1 : 0 );
    if (eqno_box != null && swap_dir)
        l = !l;
    adjust_tail = adjust_head;
    pre_adjust_tail = pre_adjust_head;
    eq_box = hpack(p, 0, additional, direction_unknown);
    ...
    if (eq_w + eqno_w2 > line_w) {
        ...
        */
        if ((eqno_w != 0) && ((eq_w - total_shrink[normal] + eqno_w2 <= line_w)
            ...
            flush_node(eq_box);
            eq_box = hpack(p, line_w - eqno_w2, exactly, direction_unknown);
            subtype(eq_box) = equation_list;
            build_attribute_list(eq_box);
        } else {
            eqno_w = 0;
            if (eq_w > line_w) {
                list_ptr(eq_box) = null;
                flush_node(eq_box);
                eq_box = hpack(p, line_w, exactly, direction_unknown);
                subtype(eq_box) = equation_list;
                build_attribute_list(eq_box);
            }
        }
    ...
    /*tex
        ...
        equation number. If that would make it too close (where \quote {too close}
    ...
    */
    ...
    /*tex
        ...
        |\leqno| on a forced single line due to |width=0|; it follows that
        |type(a) = hlist_node|
    ...
    */
    ...
    if (eqno_w != 0) {
        ...
        eq_box = hpack(eq_box, 0, additional, direction_unknown);
...
void after_math(void)
{
    ...
    halfword p = fin_mlist(null);
    if (cur_cmd == math_shift_cs_cmd && (cur_chr == text_style || cur_chr == display_style)) {
        you_cant();
    }
    if (mode == -m) {
        ...
        run_mlist_to_hlist(p, 0, text_style);
        a = hpack(vlink(temp_head), 0, additional, direction_unknown);
        build_attribute_list(a);
        unsave_math();
        /*tex now |cur_group=math_shift_group| */
        decr(save_ptr);
        if (saved_type(0) != saved_eqno) {
            confusion("after_math");
        }
        if (saved_value(0) == 1) {
            l = 1;
        }
    ...
    if (m < 0) {
        /*tex
            ...
            of |\mathsurround| inside of |$...$| affects the spacing at these
        ...
        */
        ...
        if (dir_math_save) {
            tail_append(new_dir(normal_dir,math_direction_par));
        }
        ...
        if (dir_math_save) {
            tail_append(new_dir(cancel_dir,math_direction_par));
        }
        dir_math_save = 0;
        ...
        run_mlist_to_hlist(p, 0, display_style);
...
void finish_display_alignment(halfword p, halfword q, halfword saved_prevdepth)
{
    do_assignments();
    if (cur_cmd == math_shift_cmd) {
        check_second_math_shift();
