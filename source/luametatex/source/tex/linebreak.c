...
/*tex
    ...
    mechanism for choosing the \quote {best possible} breakpoints that yield the
...
/*tex We collect the global variables in a structure: */

//linebreak_state_info linebreak_state;
...
/*tex
    ...
    and go into the enclosing semantic level. We also append the |\parfillskip|
    glue to the end of the paragraph, removing a space (or other glue node) if it
    was there, since spaces usually precede blank lines and instances of |$$|.
    The |par_fill_skip| is preceded by an infinite penalty, so it will never be
...
# define last_line_fill linebreak_state.last_line_fill

void line_break(int d, int line_break_context)
{
    ...
    lua_node_filter(pre_linebreak_filter_callback, line_break_context, temp_head, &(cur_list.tail_field));
    last_line_fill = cur_list.tail_field;
    pop_nest();
    start_of_par = cur_list.tail_field;
    if (lua_linebreak_callback(d, temp_head, &(cur_list.tail_field))) {
    ...
    lua_node_filter(post_linebreak_filter_callback, line_break_context, start_of_par, &(cur_list.tail_field));
    pack_begin_line = 0;
}
...
/*tex
    ...
    include \quote {infinite} shrinkability; that is why the algorithm maintains four
...
# define no_shrink_error_yet linebreak_state.no_shrink_error_yet

/*tex Recover from infinite shrinkage: */
...
# define second_pass linebreak_state.second_pass

/*tex is this our final attempt to break this paragraph? */

# define final_pass linebreak_state.final_pass

/*tex maximum badness on feasible lines */

# define threshold linebreak_state.threshold

/*tex

    The maximum fill level for |hlist_stack|. Maybe good if larger than |2 *
    max_quarterword|, so that box nesting level would overflow first. The stack
    for |find_protchar_left()| and |find_protchar_right()|:

*/

# define hlist_stack linebreak_state.hlist_stack

/*tex fill level for |hlist_stack| */

# define hlist_stack_level linebreak_state.hlist_stack_level
...
/*tex
    ...
    The maximal stretch and shrink ratio of expanded fonts and the the current
    step of expanded fonts:

*/

# define max_stretch_ratio linebreak_state.max_stretch_ratio
# define max_shrink_ratio  linebreak_state.max_shrink_ratio
# define cur_font_step     linebreak_state.cur_font_step

static int check_expand_pars(internal_font_number f)
{
    int m;
...
halfword find_protchar_left(halfword l, int d)
{
    int done = 0 ;
    while ((vlink(l)) && (type(l) == hlist_node) && zero_dimensions(l) && (list_ptr(l) == null)) {
    ...
    if ((!done) && d) {
        while ((vlink(l)) && (!(is_char_node(l) || non_discardable(l)))) {
    ...
    if (type(l) != glyph_node) {
        ...
        do {
            t = l;
            while (run && (type(l) == hlist_node) && (list_ptr(l))) {
                push_node(l);
                l = list_ptr(l);
            }
            while (run && cp_skipable(l)) {
                ...
                if ((vlink(l)) && (type(l) == boundary_node) && (subtype(l) == protrusion_boundary) &&
...
/*tex
    ...
    each break that is {\em feasible}, in the sense that there is a way to end a
    line at the given place without requiring any line to stretch more than a
    given tolerance. A break node is characterized by three things: the position
    of the break (which is a pointer to a |glue_node|, |math_node|,
    |penalty_node|, or |disc_node|); the ordinal number of the line that will
    follow this breakpoint; and the fitness classification of the line that has
    just ended, i.e., |tight_fit|, |decent_fit|, |loose_fit|, or
    |very_loose_fit|.
...
/*tex
    ...
    questions like, \quotation {What is the best way to break the opening part of the
    ...
    An \quote {active node} and a \quote {passive node} are created in |mem| for
    each feasible breakpoint that needs to be considered. Active nodes are three
    ...
    examined, so they are recycled within a comparatively short time after they
    are created.
...
/*tex
    ...
    The most recent node on passive list, the most recent node that has been
    printed, and the number of passive nodes allocated on this pass:

*/

# define passive      linebreak_state.passive
# define printed_node linebreak_state.printed_node
# define pass_number  linebreak_state.pass_number

/*tex

    The active list also contains \quote {delta} nodes that help the algorithm
    compute the badness of individual lines. Such nodes appear only between two
    active nodes, and they have |type=delta_node|. If |p| and |r| are active
    nodes and if |q| is a delta node between them, so that |vlink(p)=q| and
    |vlink(q)=r|, then |q| tells the space difference between lines in the
    horizontal list that start after breakpoint |p| and lines that start after
    breakpoint |r|. In other words, if we know the length of the line that starts
    after |p| and ends at our current position, then the corresponding length of
    the line that starts after |r| is obtained by adding the amounts in node~|q|.
    A delta node contains seven scaled numbers, since it must record the net
    change in glue stretchability with respect to all orders of infinity. The
    natural width difference appears in |mem[q+1].sc|; the stretch differences in
    units of pt, sfi, fil, fill, and filll appear in |mem[q+2..q+6].sc|; and the
    shrink difference appears in |mem[q+7].sc|. The |subtype| field of a delta
    node is not used.
...
/*tex
    ...
    The distance from first active node to~|cur_p|, the length of an \quote
    {empty} line, the length being computed after current break.
*/

# define active_width  linebreak_state.active_width
# define background    linebreak_state.background
# define break_width   linebreak_state.break_width

/*tex Make |auto_breaking| accessible out of |line_break|: */

# define auto_breaking linebreak_state.auto_breaking
...
# define internal_pen_inter            linebreak_state.internal_pen_inter           /*tex running |\localinterlinepenalty| */
# define internal_pen_broken           linebreak_state.internal_pen_broken          /*tex running |\localbrokenpenalty| */
# define internal_left_box             linebreak_state.internal_left_box            /*tex running |\localleftbox| */
# define internal_left_box_width       linebreak_state.internal_left_box_width      /*tex running |\localleftbox| width */
# define init_internal_left_box        linebreak_state.init_internal_left_box       /*tex running |\localleftbox| */
# define init_internal_left_box_width  linebreak_state.init_internal_left_box_width /*tex running |\localleftbox| width */
# define internal_right_box            linebreak_state.internal_right_box           /*tex running |\localrightbox| */
# define internal_right_box_width      linebreak_state.internal_right_box_width     /*tex running |\localrightbox| width */
# define disc_width                    linebreak_state.disc_width                   /*tex the length of discretionary material preceding a break */
...
# define minimal_demerits linebreak_state.minimal_demerits

/*tex best total demerits known for current line class and position */

# define minimum_demerits linebreak_state.minimum_demerits

/*tex how to achieve  |minimal_demerits| */

# define best_place linebreak_state.best_place

/*tex corresponding line number */

# define best_pl_line linebreak_state.best_pl_line
...
# define do_last_line_fit linebreak_state.do_last_line_fit

/*tex infinite stretch components of |par_fill_skip| */

# define fill_width linebreak_state.fill_width

/*tex |shortfall|  corresponding to |minimal_demerits| */

# define best_pl_short linebreak_state.best_pl_short

/*tex corresponding glue stretch or shrink */

# define best_pl_glue linebreak_state.best_pl_glue
...
static void add_to_widths(halfword s, int adjust_spacing, scaled * widths)
{
    while (s) {
        if (is_char_node(s)) {
            widths[1] += glyph_width_ex(s);
...
static void sub_from_widths(halfword s, int adjust_spacing, scaled * widths)
{
    while (s) {
...
static void compute_break_width(int break_type, int adjust_spacing, halfword p)
{
    /*tex

        Glue and other whitespace to be skipped after a break; used if
        unhyphenated, or |post_break=null|.
    ...
    */
    ...
    if (break_type > unhyphenated_node && p != null) {
        ...
        */
        sub_from_widths(vlink_no_break(p), adjust_spacing, break_width);
        add_to_widths(vlink_post_break(p), adjust_spacing, break_width);
...
static void print_feasible_break(halfword cur_p, halfword r, halfword b, int pi, int d, int artificial_demerits)
{
    /*tex
...
# define update_width(a) cur_active_width[a] += vvalue(r+(a))

# define set_break_width_to_background(a) break_width[a]=background[(a)]

# define convert_to_break_width(a) \
    vvalue(prev_r+(a)) = vvalue(prev_r+(a)) - cur_active_width[(a)]+break_width[(a)]

# define store_break_width(a) active_width[(a)]=break_width[(a)]

# define new_delta_to_break_width(a) \
    vvalue(q+(a)) = break_width[(a)]-cur_active_width[(a)]

# define new_delta_from_break_width(a) \
    vvalue(q+(a)) = cur_active_width[(a)]-break_width[(a)]

# define copy_to_cur_active(a) cur_active_width[(a)]=active_width[(a)]

# define combine_two_deltas(a) vvalue(prev_r+(a)) += vvalue(r+(a))
# define downdate_width(a) cur_active_width[(a)] -= vvalue(prev_r+(a))
# define update_active(a) active_width[(a)] += vvalue(r+(a))
            ...
            if ((minimum_demerits < awful_bad)
                ...
                */
                if (no_break_yet) {
                    ...
                    compute_break_width(break_type, adjust_spacing, cur_p);
                }
                /*tex
                ...
                */
                ...
                /*tex
                    ...
                    at least one active node before |r|, so |type(prev_r) <>
                    delta_node|.
            ...
            */
            if (l > easy_line) {
                ...
                if (l > last_special_line) {
                    ...
                    line_width = vvalue(par_shape_ptr + 2 * l + 1);
                }
            }
        }
        /*tex
        ...
        */
        ...
        if (protrude_chars > 1) {
            ...
            if (cur_p == null) {
                ...
                o = alink(cur_p);
                if (vlink(o) != cur_p) {
                    normal_error("linebreak","the node list is messed up");
                }
            }
            /*tex
        ...
        */
        ...
        if (prev_r == active) {
            /*tex
                ...
                deleted. The following code uses the fact that |type(active) <>
...
)
{
 ...
 /* int line_break_dir = paragraph_dir; */
    (void)(inter_line_penalties_ptr); /* avoid not used message */
    ...
    */
    if (par_shape_ptr == null) {
        ...
        second_indent = vvalue(par_shape_ptr + 2 * (last_special_line + 1));
        second_width = vvalue(par_shape_ptr + 2 * (last_special_line + 1) + 1);
            ...
            */
            while (is_char_node(cur_p)) {
                /*tex
                    ...
                    streamlined for speed. We use the fact that |\parfillskip|
                    glue appears at the end of each paragraph; it is therefore
                    unnecessary to check if |vlink(cur_p)=null| when |cur_p| is a
                    character node.

                */
                active_width[1] += glyph_width_ex(cur_p);
                    ...
                    */
                    if (second_pass || subtype(cur_p) <= automatic_disc) {
                        ...
                        if (s == null) {
                            ...
                            add_to_widths(s, adjust_spacing, disc_width);
                    ...
                    add_to_widths(s, adjust_spacing, active_width);
                    break;
                case penalty_node:
                    ext_try_break(
                        penalty(cur_p),
  ...
  DONE:
    ...
    /*tex
        ...
        has been found (hurray), we call on the procedure |post_line_break| to
        finish the remainder of the work. By introducing this subprocedure, we
        are able to keep |line_break| from getting extremely long. The first
        thing |ext_post_line_break| does is reset |dir_ptr|.
    */
    /*
    flush_node_list(dir_ptr);
    dir_ptr = null;
