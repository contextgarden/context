...
/*tex This will become parameters or so. */

typedef struct math_state_info {
    internal_font_number cur_f; /*tex the |font| field of a |math_char| */
    int cur_c;                  /*tex the |character| field of a |math_char| */
    int cur_size;               /*tex Size code corresponding to |cur_style|:  */
} math_state_info ;

static math_state_info math_state;
...
{
    char msg[256];
    snprintf(msg, 256, "Math error: parameter \\Umath%s\\%sstyle is not set", param, math_style_names[style]);
    tex_error(
        msg,
...
/*tex
    ...
    The non-staticness of this function is for the benefit of |texmath.w|. Watch
    out, this one uses the style! The style and size numbers don't match because
    we have cramped styles.
...
# define minimum_operator_size(var) get_math_param(math_param_operator_size, var)
...
# define radical_rule_par(var) get_math_param(math_param_radical_rule, var)
...
# define get_math_param_or_zero(a,b) do_get_math_param_or_zero(a, math_param_##b)
...
static scaled do_get_math_param_or_zero(int var, int param)
{
    scaled a = get_math_param(param, var);
    if (a == undefined_math_parameter) {
        return 0;
...
/*tex A variant on a suggestion on the list based on analysis by UV. */

static scaled get_delimiter_height(scaled max_d, scaled max_h, int axis, int cur_size) {
...
# define setup_cur_size(a) do { \
    if (a==script_style || a==cramped_script_style) \
        math_state.cur_size = script_size; \
    else if (a==script_script_style || a==cramped_script_script_style) \
        math_state.cur_size = script_script_size; \
    else \
        math_state.cur_size = text_size; \
} while (0)
...
static halfword math_clone(halfword q)
{
    halfword x;
    if (q == null)
        return null;
...
static halfword do_fraction_rule(scaled t, halfword att, halfword some_rule, halfword cur_size, halfword cur_fam)
{
    halfword p;
    if (math_rules_mode_par) {
        p = new_rule(some_rule);
        rule_data(p) = fam_fnt(cur_fam, cur_size);
    } else {
...
static halfword overbar(halfword b, scaled k, scaled t, scaled ht, halfword att, halfword index, halfword cur_size, halfword cur_fam)
{
    halfword p, q;
    p = new_kern(k);
    reset_attributes(p, att);
...
static halfword char_box(internal_font_number f, int c, halfword bb)
{
    /*tex The new box and its character node. */
    halfword b, p;
    b = new_null_box();
    if (do_new_math(f))
        width(b) = char_width_from_font(f, c);
    else
        width(b) = char_width_from_font(f, c) + char_italic_from_font(f, c);
    height(b) = char_height_from_font(f, c);
    depth(b) = char_depth_from_font(f, c);
    subtype(b) = math_char_list ;
    reset_attributes(b, bb);
    p = new_glyph(f, c, 0); /* todo: data */
...
{
    return (char_height_from_font(f, c) + char_depth_from_font(f, c));
}
...
static scaled stack_into_box(halfword b, internal_font_number f, int c)
{
    ...
    if (type(b) == vlist_node) {
        ...
        if (depth(b) < depth(p))
            depth(b) = depth(p);
        return char_width_from_font(f, c);
    }
}

static void stack_glue_into_box(halfword b, scaled min, scaled max) {
...
static halfword get_delim_box(internal_font_number fnt, halfword chr, scaled v, scaled min_overlap, int horizontal, halfword att)
{
...
halfword make_extensible(internal_font_number fnt, halfword chr, scaled v, scaled min_overlap, int horizontal, halfword att)
{
    /*tex new box */
    halfword b;
    ...
    if (horizontal) {
        ...
        ext = char_hor_variants_from_font(fnt,chr);
    } else {
        type(b) = (quarterword) vlist_node;
        ext = char_vert_variants_from_font(fnt,chr);
    }
    if (att != null) {
        ...
        for (cur = ext; cur != NULL; cur = cur->next) {
            if (cur->extender == 0) {
                ...
                if (a == 0) {
                    /*tex for tfm fonts */
                    if (horizontal) {
                        a = char_width_from_font(fnt, cur->glyph);
                    } else {
...
{
    char msg[256];
    snprintf(msg, 256, "Math error: endless loop in charlist (U+%04x in %s)", (int) y, font_name(g));
    tex_error(
        msg,
...
static halfword do_delimiter(halfword d, int s, scaled v, int flat, int cur_style, int shift, int *stack, scaled *delta, int *same)
{
            ...
            if (g != null_font) {
              ...
              CONTINUE:
                ...
                if (char_exists(g, y)) {
                    ...
                    if (char_tag_from_font(g, y) == ext_tag) {
                        f = g;
                        c = y;
                        do_parts = 1;
                        goto FOUND;
                    ...
                    if (char_tag_from_font(g, y) == list_tag) {
                        y = char_remainder_from_font(g, y);
                        goto CONTINUE;
                    }
                }
            }
  ...
  FOUND:
    ...
    if (f != null_font) {
        ...
        */
        ext = NULL;
        if ((do_parts) && ((!flat && (ext = char_vert_variants_from_font(f,c)) != NULL)
                       ||  ( flat && (ext = char_hor_variants_from_font (f,c)) != NULL))) {
            parts_done = 1;
            if (flat) {
            ...
            if (delta != NULL) {
                if (do_new_math(f)) {
                    *delta = char_vert_italic_from_font(f,x);
                } else {
                    *delta = char_italic_from_font(f,x);
                }
            }
            if (stack != NULL)
                *stack = 1 ;
            ...
            if (!do_new_math(f)) {
                /*tex Italic gets added to width. */
                width(b) += char_italic_from_font(f, c);
            }
            if (delta != NULL) {
                /*tex This used to be (f, x). */
                *delta = char_italic_from_font(f, c);
            }
            if (stack != NULL)
                *stack = 0 ;
        }
...
static halfword rebox(halfword b, scaled w)
{
    ...
    if ((width(b) != w) && (list_ptr(b) != null)) {
        if (type(b) == vlist_node) {
            p = hpack(b, 0, additional, direction_unknown);
            reset_attributes(p, node_attr(b));
            b = p;
        }
        p = list_ptr(b);
        ...
        if ((is_char_node(p)) && (vlink(p) == null)) {
            f = font(p);
            v = char_width_from_font(f, character(p));
            if (v != width(b)) {
        ...
        reset_attributes(q, att);
        couple_nodes(p,q);
        r = hpack(b, w, exactly, direction_unknown);
...
static halfword math_glue(halfword g, scaled m)
{
    /*tex integer part of |m| */
    int n = x_over_n(m, unity);
    /*tex fraction part of |m| */
    scaled f = scanner_state.tex_remainder;
...
static void math_glue_to_glue(halfword p, scaled m)
{
    /*tex integer part of |m| */
    int n = x_over_n(m, unity);
    /*tex fraction part of |m| */
...
*/
static void math_kern(halfword p, scaled m)
{
    if (subtype(p) == mu_glue) {
        int n = x_over_n(m, unity);             /*tex integer part of |m| */
        scaled f = scanner_state.tex_remainder; /*tex fraction part of |m| */
        if (f < 0) {
            decr(n);
            f = f + unity;
        }
...
void run_mlist_to_hlist(halfword p, int penalties, int mstyle)
{
    if (p) {
        int callback_id = callback_defined(mlist_to_hlist_callback);
        int top;
        ...
        if (callback_id > 0 && (top = callback_okay(Luas, callback_id))) {
            ...
            i = callback_call(Luas,3,1,top);
            if (i != 0) {
                callback_error(Luas,top,i);
                vlink(temp_head) = null;
            } else {
                halfword a = nodelist_from_lua(Luas,-1);
                /* alink(vlink(a)) = null; */
                vlink(temp_head) = a;
...
static halfword clean_box(halfword p, int s, int cur_style, halfword st)
{
    ...
    mlist_to_hlist(mlist, 0, s);
    /*tex recursive call */
    q = vlink(temp_head);
    setup_cur_size(cur_style);
  FOUND:
    if (is_char_node(q) || (q == null))
        x = hpack(q, 0, additional, direction_unknown);
...
{
    math_state.cur_c = math_character(a);
    math_state.cur_f = fam_fnt(math_fam(a), math_state.cur_size);
    if (math_state.cur_f == null_font) {
        char msg[256];
        snprintf(msg, 255, "\\%s%d is undefined (character %d)", math_size_string(math_state.cur_size), (int) math_fam(a), (int) math_state.cur_c);
        tex_error(
            msg,
    ...
    } else if (!(char_exists(math_state.cur_f, math_state.cur_c))) {
        char_warning(math_state.cur_f, math_state.cur_c);
    }
}
...
static void assign_new_hlist(halfword q, halfword r)
{
    switch (type(q)) {
        case fraction_noad:
...
static void make_over(halfword q, int cur_style, int cur_size, int cur_fam)
{
    /*tex
...
static void make_under(halfword q, int cur_style, int cur_size, int cur_fam)
{
    /*tex
...
static void make_vcenter(halfword q, int cur_size)
{
...
static void make_hextension(halfword q, int cur_style, int cur_size)
{
    halfword e, p, w;
    int stack = 0;
    e = do_delimiter(left_delimiter(q), cur_size, radicalwidth(q), 1, cur_style, 1, &stack, NULL, NULL);
    w = width(e);
    ...
    e = hpack(e, 0, additional, direction_unknown);
    width(e) = w ;
...
static void make_radical(halfword q, int cur_style, int cur_size)
{
    /*tex temporary registers for box construction */
    halfword x, y, p, l1, l2;
    ...
    */
    ...
    if (theta == undefined_math_parameter) {
        ...
        y = do_delimiter(left_delimiter(q), cur_size, height(x) + depth(x) + clr + theta, 0, cur_style, 1, NULL, NULL, NULL);
        /*tex
        ...
        */
        ...
        if ((l1 != null) && (type(l1) == hlist_node)) {
            ...
            if ((l2 != null) && (type(l2) == glyph_node)) {
                /*tex top character */
                theta = char_height_from_font(font(l2), character(l2));
            } else {
                theta = height(y);
            }
        } else {
    ...
    */
    ...
    if (degree(q) != null) {
        scaled wr, br, ar;
        halfword r = clean_box(degree(q), script_script_style, cur_style, math_degree_list);
    ...
    p = hpack(y, 0, additional, direction_unknown);
    reset_attributes(p, node_attr(q));
    math_list(nucleus(q)) = p;
    type(nucleus(q)) = sub_box_node;
}
...
static halfword wrapup_over_under_delimiter(halfword x, halfword y, halfword q, scaled shift_up, scaled shift_down, halfword st)
{
    halfword p;
    halfword v = new_null_box();
    type(v) = vlist_node;
    subtype(v) = st;
...
# define check_radical(q,stack,r,t) do { \
    if (!stack && (width(r) >= width(t)) && (radicalwidth(q) != 0) && (radicalwidth(q) != width(r))) { \
        if (radicalleft(q)) { \
            ...
            reset_attributes(p, node_attr(q)); \
            couple_nodes(p,r); \
            r = hpack(p, 0, additional, direction_unknown); \
            ...
            r = hpack(r, 0, additional, direction_unknown); \
            width(r) = radicalwidth(q); \
            reset_attributes(r, node_attr(q)); \
        } \
    } \
...
static void make_over_delimiter(halfword q, int cur_style, int cur_size)
{
    halfword x, y, v;
    ...
    check_widths(q,x);
    y = do_delimiter(left_delimiter(q), cur_size, wd, 1, cur_style, 1, &stack, NULL, NULL);
    left_delimiter(q) = null;
...
static void make_under_delimiter(halfword q, int cur_style, int cur_size)
{
    halfword x, y, v;
    scaled shift_up, shift_down, clr, delta, wd;
    int stack;
    ...
    check_widths(q,y);
    x = do_delimiter(left_delimiter(q), cur_size, wd, 1, cur_style, 1, &stack, NULL, NULL);
    left_delimiter(q) = null;
...
static void make_delimiter_over(halfword q, int cur_style, int cur_size)
{
    halfword x, y, v;
    ...
    check_widths(q,y);
    x = do_delimiter(left_delimiter(q), cur_size + (cur_size == script_script_size ? 0 : 1), wd, 1, cur_style, 1, &stack, NULL, NULL);
    left_delimiter(q) = null;
    check_radical(q,stack,x,y);
    fixup_widths(q, x, y);
...
static void make_delimiter_under(halfword q, int cur_style, int cur_size)
{
    halfword x, y, v;
    scaled shift_up, shift_down, clr, actual, wd;
    int stack;
    x = clean_box(nucleus(q), cur_style, cur_style, math_nucleus_list);
    check_widths(q,x);
    y = do_delimiter(left_delimiter(q), cur_size + (cur_size == script_script_size ? 0 : 1), wd, 1, cur_style, 1, &stack, NULL, NULL);
    left_delimiter(q) = null;
    check_radical(q,stack,y,x);
    fixup_widths(q, x, y);
    shift_up = 0;
...
static int compute_accent_skew(halfword q, int flags, scaled *s)
{
    ...
    if (type(nucleus(q)) == math_char_node) {
        fetch(nucleus(q));
        if (do_new_math(math_state.cur_f)) {
            /*tex
                There is no bot_accent so let's assume similarity

                \starttyping
                if (flags & (TOP_CODE | OVERLAY_CODE)) {
                    *s = char_top_accent_from_font(math_state.cur_f,math_state.cur_c);
                    if (*s != INT_MIN) {
                        s_is_absolute = 1;
                    }
                } else {
                    *s = char_bot_accent_from_font(math_state.cur_f,math_state.cur_c);
                    if (*s != INT_MIN) {
                        s_is_absolute = 1;
                    }
                }
            ...
            *s = char_top_accent_from_font(math_state.cur_f,math_state.cur_c);
            ...
            if (flags & TOP_CODE) {
                *s = get_kern(math_state.cur_f, math_state.cur_c, skew_char(math_state.cur_f));
            } else {
                *s = 0;
            }
        }
...
static void do_make_math_accent(halfword q, internal_font_number f, int c, int flags, int cur_style, int cur_size)
{
    /*tex temporary registers for box construction */
    halfword p, r, x, y;
    ...
    w = width(x);
    h = height(x);
    if (do_new_math(math_state.cur_f) && !s_is_absolute) {
        s = half(w);
        s_is_absolute = 1;
    }
    ...
    if ((flags & STRETCH_ACCENT_CODE) && (char_width_from_font(f, c) < w)) {
        while (1) {
            if ((char_tag_from_font(f, c) == ext_tag) && ((ext = char_hor_variants_from_font(f,c)) != NULL)) {
                ...
                break;
            } else if (char_tag_from_font(f, c) != list_tag) {
                break;
            } else {
                int yy = char_remainder_from_font(f, c);
                if (!char_exists(f, yy)) {
                    break;
                } else if (flags & OVERLAY_CODE) {
                    if (char_height_from_font(f, yy) > target) {
                        break;
                    }
                } else {
                    if (char_width_from_font(f, yy) > target)
                    break;
                }
                c = yy;
            }
    ...
    if ((supscr(q) != null) || (subscr(q) != null)) {
        ...
        internal_font_number fnum = fam_fnt(math_fam(nucleus(q)),cur_size);
        if (do_new_math(fnum)) {
            ic = char_italic_from_font(fnum,math_character(nucleus(q)));
        }
    }
    ...
    if (s_is_absolute) {
        ...
        if (ext != NULL) {
            ...
            /*tex
                ...
                \starttyping
                if (flags & BOT_CODE) {
                    sa = char_bot_accent_from_font(f, c);
                } else {
                    sa = char_top_accent_from_font(f, c);
                }
                \stoptyping
            */
            sa = char_top_accent_from_font(f, c);
...
static void make_math_accent(halfword q, int cur_style, int cur_size)
{
    ...
    if (top_accent_chr(q) != null) {
        ...
        if (char_exists(math_state.cur_f, math_state.cur_c)) {
          do_make_math_accent(q, math_state.cur_f, math_state.cur_c, TOP_CODE | (topstretch ? STRETCH_ACCENT_CODE : 0), cur_style, cur_size);
        }
    ...
    if (bot_accent_chr(q) != null) {
        ...
        if (char_exists(math_state.cur_f, math_state.cur_c)) {
          do_make_math_accent(q, math_state.cur_f, math_state.cur_c, BOT_CODE | (botstretch ? STRETCH_ACCENT_CODE : 0), cur_style, cur_size);
        }
    ...
    if (overlay_accent_chr(q) != null) {
        ...
        if (char_exists(math_state.cur_f, math_state.cur_c)) {
          do_make_math_accent(q, math_state.cur_f, math_state.cur_c, OVERLAY_CODE | STRETCH_ACCENT_CODE, cur_style, cur_size);
        }
...
static void make_fraction(halfword q, int cur_style, int cur_size)
{
    halfword p, p1, p2, v, x, y, z, l, r, m;
    ...
    */
    ...
    if (thickness(q) == preset_rule_thickness)
    ...
    */
    ...
    if (middle_delimiter(q) != null) {
        delta = 0;
        m = do_delimiter(middle_delimiter(q), cur_size, delta, 0, cur_style, 1, NULL, NULL, NULL);
    ...
    */
    if (do_new_math(math_state.cur_f)) {
        ...
        if (delta == undefined_math_parameter) {
            delta = get_delimiter_height(depth(v), height(v), 1, cur_size);
        }
    } else {
        delta = fraction_del_size_old(cur_style);
    }
    l = do_delimiter(left_delimiter(q), cur_size, delta, 0, cur_style, 1, NULL, NULL, NULL);
    left_delimiter(q) = null;
    r = do_delimiter(right_delimiter(q), cur_size, delta, 0, cur_style, 1, NULL, NULL, NULL);
    right_delimiter(q) = null;
    couple_nodes(l,v);
    couple_nodes(v,r);
    y = hpack(l, 0, additional, direction_unknown);
    reset_attributes(y, node_attr(q));
...
static void make_scripts(halfword q, halfword p, scaled it, int cur_style, scaled supshift, scaled subshift);
static halfword check_nucleus_complexity(halfword q, scaled * delta, int cur_style, int *same);

static scaled make_op(halfword q, int cur_style, int cur_size)
{
    /*tex offset between subscript and superscript */
    scaled delta = 0;
    scaled dummy = 0;
    /*tex temporary registers for box construction */
    halfword p, v, x, y, z, n;
    ...
    if (type(nucleus(q)) == math_char_node) {
        ...
        if (cur_style < text_style) {
            ...
            if (ok_size != undefined_math_parameter) {
                ...
                x = do_delimiter(y, text_size, ok_size, 0, cur_style, 1, NULL, &delta, NULL);
                if (delta != 0) {
                    if (do_new_math(math_state.cur_f)) {
                ...
                ok_size = height_plus_depth(math_state.cur_f, math_state.cur_c) + 1;
                while ((char_tag_from_font(math_state.cur_f,math_state.cur_c) == list_tag) && height_plus_depth(math_state.cur_f,math_state.cur_c) < ok_size) {
                    c = char_remainder_from_font(math_state.cur_f,math_state.cur_c);
                    if (!char_exists(math_state.cur_f,c))
                        break;
                    math_state.cur_c = c;
                    math_character(nucleus(q)) = c;
                }
                delta = char_italic_from_font(math_state.cur_f,math_state.cur_c);
                x = clean_box(nucleus(q), cur_style, cur_style, math_nucleus_list);
                if (delta != 0) {
                    if (do_new_math(math_state.cur_f)) {
                        /*tex we never added italic correction */
            ...
            if (n != null) {
                if ((type(n) == sub_mlist_node) || (type(n) == sub_box_node)) {
                        ...
                        if (type(n) == hlist_node) {
                            ...
                            while (n != null) {
                                if (type(n) == glyph_node) {
                                    delta = char_italic_from_font(font(n),character(n));
                                }
                                n = vlink(n);
                            }
                        } else {
                    ...
                    if (type(n) == math_char_node) {
                        delta = char_italic_from_font(fam_fnt(math_fam(n),cur_size),math_character(n));
                    }
                }
            }
        }
...
static void make_ord(halfword q)
{
  ...
  RESTART:
    if (subscr(q) == null && supscr(q) == null && type(nucleus(q)) == math_char_node) {
        ...
        if (simple_char_noad(p) && same_nucleus_fam(p,q)) {
            ...
            if (do_new_math(math_state.cur_f) && (char_italic_from_font(math_state.cur_f,math_character(nucleus(q))) != 0)) {
                p = new_kern(char_italic_from_font(math_state.cur_f,math_character(nucleus(q))));
                subtype(p) = italic_kern;
                reset_attributes(p, node_attr(q));
            ...
            if ((has_kern(math_state.cur_f,a)) || (has_lig(math_state.cur_f,a))) {
                math_state.cur_c = math_character(nucleus(p));
                /*tex
                ...
                */
                if (disable_lig_par == 0 && has_lig(math_state.cur_f,a)) {
                    lig = get_ligature(math_state.cur_f,a,math_state.cur_c);
                    if (is_valid_ligature(lig)) {
                        ...
                        default:
                            ...
                            math_character(nucleus(q)) = lig_replacement(lig); /*tex |=:| */
                            subscr(q) = subscr(p);
                            supscr(q) = supscr(p);
                            subscr(p) = null ;
                            supscr(p) = null ;
                ...
                if (disable_kern_par == 0 && has_kern(math_state.cur_f,a)) {
                    /*tex todo: should this use mathkerns? */
                    k = get_kern(math_state.cur_f,a,math_state.cur_c);
                    if (k != 0) {
                        p = new_kern(k);
                        reset_attributes(p, node_attr(q));
                        couple_nodes(p,vlink(q));
...
static scaled find_math_kern(internal_font_number l_f, int l_c, internal_font_number r_f, int r_c, int cmd, scaled shift)
{
    ...
    if (cmd == sup_mark_cmd) {
        corr_height_top = char_height_from_font(l_f, l_c);
        /*tex bottom of superscript */
        corr_height_bot = -char_depth_from_font(r_f, r_c) + shift;
        ...
        corr_height_top = char_height_from_font(r_f, r_c) - shift;
        corr_height_bot = -char_depth_from_font(l_f, l_c);
...
static halfword attach_hkern_to_new_hlist(halfword q, scaled delta2, halfword subtyp)
{
    halfword y;
    halfword z = new_kern(delta2);
    if (subtyp != 0) {
        subtype(z) = subtyp;
    }
...
# define analyze_script(init,su_n,su_f,su_c) do { \
    ...
    if (su_n != null) { \
        if (math_script_char_mode_par > 0 && type(su_n) == math_char_node) { \
            fetch(su_n); \
            if (char_exists(math_state.cur_f,math_state.cur_c)) { \
                su_f = math_state.cur_f; \
                su_c = math_state.cur_c; \
            } else { \
                su_n = null; \
            } \
...
static void make_scripts(halfword q, halfword p, scaled it, int cur_style, scaled supshift, scaled subshift)
{
    ...
    if (is_char_node(p)) {
        ...
        z = hpack(p, 0, additional, direction_unknown);
        ...
        */
        ...
        if (subscr(q) == null) {
            ...
            /*tex
                ...
                superscript by at least four times |preset_rule_thickness| If
...
static halfword make_left_right(halfword q, int style, scaled max_d, scaled max_h, int cur_size)
{
    scaled delta;
    halfword tmp, lst;
    scaled ic = 0;
    ...
    if ((delimiterheight(q)!=0) || (delimiterdepth(q)!=0)) {
        ...
        tmp = do_delimiter(delimiter(q), cur_size, delta, 0, style, 0, &stack, &ic, &same);
        ...
        */
        ...
        box_dir(lst) = (quarterword) dir_lefttoright ;
        ...
        delta = get_delimiter_height(max_d,max_h,axis,cur_size);
        tmp = do_delimiter(delimiter(q), cur_size, delta, 0, style, axis, &stack, &ic, &same);
        delimiteritalic(q) = ic;
    }
    delimiter(q) = null;
...
static halfword math_spacing_glue(int l_type, int r_type, int mstyle, scaled mmu)
{
    int x = -1;
    halfword z = null;
    ...
    switch (type(nucleus(q))) {
        ...
        case math_text_char_node:
            ...
            if (char_exists(math_state.cur_f,math_state.cur_c)) {
                ...
                p = new_glyph(math_state.cur_f,math_state.cur_c,0); /* todo: data */
                reset_attributes(p, node_attr(nucleus(q)));
                if (do_new_math(math_state.cur_f)) {
                    if (get_char_cat_code(math_state.cur_c) == 11) {
                        /*tex no italic correction in mid-word of text font */
                        *delta = 0;
                    }
                } else {
                    /*tex no italic correction in mid-word of text font */
                    if (((type(nucleus(q))) == math_text_char_node) && (space(math_state.cur_f) != 0)) {
                        *delta = 0;
                    }
                }
                /*tex so we only add italic correction when we have no scripts */
                if ((subscr(q) == null) && (supscr(q) == null) && (*delta != 0)) {
                    halfword x = new_kern(*delta);
                    subtype(x) = italic_kern;
                    reset_attributes(x, node_attr(nucleus(q)));
                    couple_nodes(p,x);
                    *delta = 0;
                } else if (do_new_math(math_state.cur_f)) {
        ...
        case sub_mlist_node:
            ...
            /*tex Recursive call: */
            mlist_to_hlist(t, 0, cur_style);
            if (same != NULL && type(t) == fence_noad && delimitersamesize(t)) {
                *same = delimitersamesize(t) ;
            }
            setup_cur_size(cur_style);
            p = hpack(vlink(temp_head), 0, additional, direction_unknown);
            reset_attributes(p, node_attr(nucleus(q)));
            break;
        default:
            confusion("mlist2");
...
void mlist_to_hlist(halfword mlist, int penalties, int cur_style)
{
    ...
    setup_cur_size(cur_style);
    cur_mu = x_over_n(get_math_quad_size(math_state.cur_size), 18);
    ...
    while (q != null) {
      ...
      RESWITCH:
        ...
        switch (type(q)) {
            case simple_noad:
                switch (subtype(q)) {
                    case bin_noad_type:
                        ...
                        break;
                    case over_noad_type:
                        make_over(q, cur_style, math_state.cur_size, math_rules_fam_par);
                        break;
                    case under_noad_type:
                        make_under(q, cur_style, math_state.cur_size, math_rules_fam_par);
                        break;
                    case vcenter_noad_type:
                        make_vcenter(q, math_state.cur_size);
                        break;
                    case rel_noad_type:
                    ...
                    case op_noad_type_limits:
                    case op_noad_type_no_limits:
                        delta = make_op(q, cur_style, math_state.cur_size);
            ...
            case fence_noad:
                ...
                break;
            case fraction_noad:
                make_fraction(q, cur_style, math_state.cur_size);
                goto CHECK_DIMENSIONS;
                break;
            case radical_noad:
                if (subtype(q) == 7)
                    make_hextension(q, cur_style, math_state.cur_size);
                else if (subtype(q) == 6)
                    make_delimiter_over(q, cur_style, math_state.cur_size);
                else if (subtype(q) == 5)
                    make_delimiter_under(q, cur_style, math_state.cur_size);
                else if (subtype(q) == 4)
                    make_over_delimiter(q, cur_style, math_state.cur_size);
                else if (subtype(q) == 3)
                    make_under_delimiter(q, cur_style, math_state.cur_size);
                else
                    make_radical(q, cur_style, math_state.cur_size);
                break;
            case accent_noad:
                make_math_accent(q, cur_style, math_state.cur_size);
                break;
            case style_node:
                cur_style = subtype(q);
                setup_cur_size(cur_style);
            ...
            case glue_node:
                ...
                */
                ...
                } else if ((math_state.cur_size != text_size) && (subtype(q) == cond_math_glue)) {
                    p = vlink(q);
                    if (p != null)
        ...
        /*tex
            ...
            When we get to the following part of the program, we have \quote
            {fallen through} from cases that did not lead to |check_dimensions|
            or |done_with_noad| or |done_with_node|. Thus, |q|~points to a noad
      ...
      CHECK_DIMENSIONS:
        z = hpack(new_hlist(q), 0, additional, direction_unknown);
        if (height(z) > max_hl)
            max_hl = height(z);
        if (depth(z) > max_d)
            max_d = depth(z);
    ...
    while (q != null) {
        ...
        */
        ...
        case fraction_noad:
            ...
            break;
        case fence_noad:
            t_subtype = make_left_right(q, style, max_d, max_hl, math_state.cur_size);
            break;
        case style_node:
        ...
        /*tex
            ...
            |q| if |pen| is not an \quote {infinite} penalty, and if the node
      ...
      DELETE_Q:
        ...
        */
        ...
        free_node(r, get_node_size(type(r)));
    }
}
