...
/*tex
    ...
    and parameter |m| is either |exactly| or |additional|. Thus, |hpack(p, w,
    exactly)| produces a box whose width is exactly |w|, while |hpack(p, w,
    additional)| yields a box whose width is the natural width plus |w|. It is
    convenient to define a macro called |natural| to cover the most common case,
    so that we can say |hpack(p, natural)| to get a box that has the natural
    width of list |p|.
    ...
    |\hbox to 300pt|, |\hbox spread 10pt|; note that |\hbox| with no dimension
    following it is equivalent to |\hbox spread 0pt|. The |scan_spec| subroutine
    scans such constructions in the user's input, including the mandatory left
    brace that follows them, and it puts the specification onto |save_stack| so
...
/*tex
    ...
    exaclty the uses of |\hbox|, |\vbox|, and |\vtop| in the input stream (the
...
void scan_full_spec(group_code c, int spec_direction, int just_pack)
{
  ...
  KEYWORDS:
    ...
    /*tex We only permit one |direction| directive. */
  DIRECTION:
    if (! dir_done) {
        if (scan_keyword("direction")) {
            scan_int(0);
    ...
    /*tex We only permit one |orientation| directive, optionally followed by offsets. */
  ORIENTATION:
    if (! orientation_done) {
        if (scan_keyword("orientation")) {
            int x_offset = 0;
            int y_offset = 0;
            int x_move = 0;
            int y_move = 0;
            scan_int(0);
            check_orientation_value(cur_val);
            orientation = cur_val;
            orientation_done = 1;
          AGAIN:
            if (!x_offset && scan_keyword("xoffset")) {
                scan_normal_dimen(0);
                xoffset = cur_val;
                x_offset = 1;
                setboxoffset(orientation);
                goto AGAIN;
            }
            if (!y_offset && scan_keyword("yoffset")) {
                scan_normal_dimen(0);
                yoffset = cur_val;
                y_offset = 1;
                setboxoffset(orientation);
  ...
  FOUND:
    ...
    if (spec_direction != direction_unknown) {
        ...
        text_dir_ptr = new_dir(normal_dir,spec_direction);
    } else {
    ...
    set_saved_record(5, saved_boxorientation, 0, orientation);
    set_saved_record(6, saved_boxxoffset, 0, xoffset);
    set_saved_record(7, saved_boxyoffset, 0, yoffset);
    set_saved_record(8, saved_boxxmove, 0, xmove);
    set_saved_record(9, saved_boxymove, 0, ymove);
    save_ptr += 10;
    new_save_level(c);
    if (! done) {
        scan_left_brace();
...
/*tex
    ...
    respective stretchabilities |3pt|, |8fill|, |5fil|, |6pt|, |-3fil|, |-8fill|.
    Then the total is essentially |2fil|; and if a total additional space of 6pt
    is to be achieved by stretching, the actual amounts of stretch will be |0pt|,
    |0pt|, |15pt|, |0pt|, |-9pt|, and |0pt|, since only |fi| glue will be
...
packaging_state_info packaging_state;

/*tex

    This state collects the glue found by |hpack| or |vpack|: |total_stretch| and
    |total_shrink| and the badness of the most recently packaged box
    |last_badness|.

    If the variable |adjust_tail| is non-null, the |hpack| routine also removes
    all occurrences of |ins_node|, |mark_node|, and |adjust_node| items and
    appends the resulting material onto the list that ends at location
    |adjust_tail|.

    Tail of adjustment list is stored in |adjust_tail|. Materials in |\vadjust|
    used with |pre| keyword will be appended to |pre_adjust_tail| instead of
    |adjust_tail|.
    ...
    negative value denotes alignment |pack_begin_line|.

    Pointers to the prev and next char of an implicit kern are kept in
    |next_char_p| and prev_char_p|.
...
/*tex
    ...
    The kern stretch and shrink code was (or had become) rather weird ... the
    width field is set, and then used in a second calculation, repeatedly, so why
    is that ... maybe some some weird left-over ... anyway, the values are so
    small that in practice they are not significant at all when the backend sees
    them because a few hundred sp positive or negative are just noise there (so
...
scaled char_stretch(halfword p)
{
    ...
    if (m > 0) {
        int c = character(p);
        int ef = char_ef_from_font(f, c);
        if (ef > 0) {
            scaled dw = calc_char_width(f, c, m) - char_width_from_font(f, c);
            if (dw > 0) {
...
scaled char_shrink(halfword p)
{
    ...
    if (m > 0) {
        ...
        int ef = char_ef_from_font(f, c);
        if (ef > 0) {
            scaled dw = char_width_from_font(f, c) - calc_char_width(f, c, -m);
            if (dw > 0) {
                return round_xn_over_d(dw, ef, 1000);
            }
        }
...
scaled kern_stretch(halfword p)
{
    ...
    /*tex We use the old logic, kind of, but average the ef as we might depend on proper overlap. */
    e = (char_ef_from_font(font(l), character(l)) + char_ef_from_font(font(r), character(r)))/2 ;
    if (e == 1000) {
        x = d - w;
    } else {
...
void do_subst_font(halfword p, int ex_ratio)
{
    if (type(p) == disc_node) {
        ...
        internal_font_number f = font(p);
        int ef = char_ef_from_font(f, character(p));
        if (ef == 0)
            return;
...
scaled char_pw(halfword p, int side)
{
    ...
    if (!p || !is_char_node(p)) {
        ...
        internal_font_number f = font(p);
        if (side == left_side) {
            c = char_lp_from_font(f, character(p));
            last_leftmost_char = p;
        } else {
            c = char_rp_from_font(f, character(p));
...
{
    halfword k = new_node(margin_kern_node, side);
    width(k) = w;
    if (p) {
        margin_char(k) = new_char(font(p), character(p), glyph_data(p));
    } else {
...
static int font_expand_ratio = 0;
...
halfword hpack(halfword p, scaled w, int m, int pack_direction)
{
    ...
    if (pack_direction == direction_unknown) {
        hpack_dir = text_direction_par;
    } else {
        hpack_dir = pack_direction;
    }
    box_dir(r) = (quarterword) hpack_dir;
    /*tex
        ...
        */
        while (is_char_node(p)) {
            /*tex
                ...
                {\em NB: in \LUATEX\ ligature building, kerning and hyphenation
                are decoupled so comments about inner loop and performance no
                longer apply.}
        ...
        if (p) {
            switch (type(p)) {
                ...
                case vlist_node:
                    {
                        ...
                        */
                        halfword s = shift_amount(p);
                        scaled_whd whd = pack_dimensions(p);
                        x += whd.wd;
                        if (whd.ht - s > h)
                            h = whd.ht - s;
                        if (whd.dp + s > d)
                ...
                case adjust_node:
                    ...
                    */
                    if (adjust_tail || pre_adjust_tail) {
                        while (vlink(q) != p)
                            q = vlink(q);
                ...
                */
                ...
                if (last_badness > hbadness_par) {
                    ...
                    if (callback_id > 0) {
                        halfword rule = null;
                        if (last_badness > 100) {
                            run_callback(callback_id, "SdNdd->N","underfull",last_badness,r,abs(pack_begin_line),input_line,&rule);
                        } else {
                            run_callback(callback_id, "SdNdd->N","loose",last_badness,r,abs(pack_begin_line),input_line,&rule);
                        }
                        if (rule != null) {
                            while (vlink(q) != null) {
                                q = vlink(q);
        ...
        if ((total_shrink[o] < -x) && (o == normal) && (list_ptr(r) != null)) {
            ...
            */
            if ((overshoot > hfuzz_par) || (hbadness_par < 100)) {
                ...
                halfword rule = null;
                if (callback_id > 0) {
                    run_callback(callback_id, "SdNdd->N","overfull",overshoot,r,abs(pack_begin_line),input_line,&rule);
            ...
            if (list_ptr(r) != null) {
                ...
                */
                ...
                if (last_badness > hbadness_par) {
                    ...
                    if (callback_id > 0) {
                        halfword rule = null;
                        run_callback(callback_id, "SdNdd->N","tight",last_badness,r,abs(pack_begin_line),input_line,&rule);
                        if (rule != null) {
                            while (vlink(q) != null) {
                                q = vlink(q);
                            }
...
scaled_whd natural_sizes(halfword p, halfword pp, glue_ratio g_mult, int g_sign, int g_order)
{
    ...
    while ((p) && p != pp) {
        while (is_char_node(p) && p != pp) {
            scaled_whd whd = glyph_dimensions(p);
            siz.wd += whd.wd;
            if (whd.ht > siz.ht)
                siz.ht = whd.ht;
            if (whd.dp > siz.dp)
        ...
        if ((p) && p != pp) {
            switch (type(p)) {
                    ...
                    if (g_sign != normal) {
                        if (g_sign == stretching) {
                            if (stretch_order(p) == g_order) {
                                /*tex
                                    |siz.wd += round(float_cast(g_mult) * float_cast(stretch(p)))|
                                */
                                gp += stretch(p);
                            }
                        } else if (shrink_order(p) == g_order) {
                            /*tex
                                |siz.wd -= round(float_cast(g_mult) * float_cast(shrink(p)));|
                            */
                            gm += shrink(p);
                        }
                    }
                ...
                case disc_node:
                    {
                        scaled_whd whd = natural_sizes(no_break(p), null, g_mult, g_sign, g_order);
                        siz.wd += whd.wd;
                        if (whd.ht > siz.ht)
                            siz.ht = whd.ht;
                        if (whd.dp > siz.dp)
    ...
    if (g_sign != normal) {
        if (g_sign == stretching) {
            siz.wd += round(float_cast(g_mult) * float_cast(gp));
        } else {
            siz.wd -= round(float_cast(g_mult) * float_cast(gm));
        }
    }
    return siz;
}
...
halfword natural_width(halfword p, halfword pp, glue_ratio g_mult, int g_sign, int g_order)
{
    scaled wd = 0;
    scaled gp = 0;
    scaled gm = 0;
        ...
        if ((p) && p != pp) {
            switch (type(p)) {
                ...
                case glue_node:
                    ...
                    break;
                case kern_node:
                    wd += width(p) + ex_kern(p);
                    break;
                case disc_node:
                    wd += natural_width(no_break(p), null, g_mult, g_sign, g_order);
                    break;
                default:
                    break;
            }
    ...
    if (g_sign != normal) {
        if (g_sign == stretching) {
            wd += round(float_cast(g_mult) * float_cast(gp));
        } else {
            wd -= round(float_cast(g_mult) * float_cast(gm));
        }
    }
    return wd;
}
...
halfword vpackage(halfword p, scaled h, int m, scaled l, int pack_direction)
{
    ...
    box_dir(r) = (quarterword) pack_direction;
        ...
        if (o == normal && list_ptr(r)) {
            ...
            */
            ...
            if (last_badness > vbadness_par) {
                ...
                if (callback_id > 0) {
                    if (last_badness > 100) {
                        run_callback(callback_id, "SdNdd->","underfull",last_badness,r,abs(pack_begin_line),input_line);
                    } else {
                        run_callback(callback_id, "SdNdd->","loose",last_badness,r,abs(pack_begin_line),input_line);
                    }
                    goto EXIT;
                } else {
                    print_ln();
        ...
        if ((total_shrink[o] < -x) && (o == normal) && (list_ptr(r) != null)) {
            ...
            */
            if ((overshoot > vfuzz_par) || (vbadness_par < 100)) {
                int callback_id = callback_defined(vpack_quality_callback);
                if (callback_id > 0) {
                    run_callback(callback_id, "SdNdd->","overfull",overshoot,r,abs(pack_begin_line),input_line);
                    goto EXIT;
                } else {
                    print_ln();
            ...
            if (list_ptr(r) != null) {
                ...
                */
                ...
                if (last_badness > vbadness_par) {
                    int callback_id = callback_defined(vpack_quality_callback);
                    if (callback_id > 0) {
                        run_callback(callback_id, "SdNdd->","tight",last_badness,r,abs(pack_begin_line),input_line);
                        goto EXIT;
                    } else {
                        print_ln();
...
void finish_vcenter(void)
{
    ...
    unsave();
    save_ptr--;
    p = vpack(vlink(cur_list.head_field), saved_value(0), saved_level(0), direction_unknown);
    pop_nest();
...
void package(int c)
{
    halfword context, spec, dirptr, attrlist, justpack, orientation; /* todo: use proper names */
    int grp = cur_group;
    scaled d = box_max_depth_par;
    unsave();
    save_ptr -= 10;
    context = saved_value(0);
    spec = saved_value(1);
    dirptr = saved_value(2);
    attrlist = saved_value(3);
    justpack = saved_value(4);
    orientation = saved_value(5);
    if (cur_list.mode_field == -hmode) {
        cur_box = filtered_hpack(cur_list.head_field, cur_list.tail_field,
            spec, saved_level(1), grp, saved_level(2), justpack, attrlist);
        subtype(cur_box) = hbox_list;
    } else {
        cur_box = filtered_vpackage(vlink(cur_list.head_field),
            spec, saved_level(1), d, grp, saved_level(2), justpack, attrlist);
        if (c == vtop_code) {
            /*tex

                Read just the height and depth of |cur_box|, for |\vtop|. The
                height of a |\vtop| box is inherited from the first item on its
                list, if that item is an |hlist_node|, |vlist_node|, or
    ...
    replace_attribute_list(cur_box, attrlist);
    if (orientation != 0) {
        halfword xoffset = saved_value(6);
        halfword yoffset = saved_value(7);
        halfword xmove = saved_value(8);
        halfword ymove = saved_value(9);
        halfword wd = width(cur_box);
        halfword ht = height(cur_box);
        halfword dp = depth(cur_box);
        if (xmove != 0) {
            check_dimen1(xoffset, xoffset + xmove);
            check_dimen2(wd, wd + xmove);
        }
        if (ymove != 0) {
            check_dimen1(yoffset, yoffset + ymove);
            check_dimen2(ht, ht + ymove);
            check_dimen2(dp, dp - ymove);
        }
        box_w_offset(cur_box) = wd;
        box_h_offset(cur_box) = ht;
        box_d_offset(cur_box) = dp;
        ...
        box_x_offset(cur_box) = xoffset;
        box_y_offset(cur_box) = yoffset;
        check_box_offsets(cur_box);
    }
    box_orientation(cur_box) = (quarterword) orientation;
    pop_nest();
    box_end(context);
}
...
void append_to_vlist(halfword b, int location)
{
    ...
    int prev_set = 0;
    if (lua_appendtovlist_callback(b,location,prev_depth_par,0,&result,&next_depth,&prev_set)) {
        while (result != null) {
        ...
        if (prev_depth_par > ignore_depth) {
            ...
            scaled d = width(baseline_skip_par) - prev_depth_par - height(b);
            halfword p;
...
halfword prune_page_top(halfword p, int s)
{
    halfword q;
...
/*tex
    ...
    the box (without stretching or shrinking) that corresponds to the optimum
    breakpoint found by |vert_break|. This value is used by the insertion
    splitting algorithm of the page builder.
...
/*tex
    ...
    The original box becomes \quote {void} if and only if it has been entirely
    extracted. The extracted box is \quote {void} if and only if the original box
    was void (or if it was, erroneously, an hlist box).
...
halfword vsplit(halfword n, scaled h, int m)
{
    ...
    /*tex
        ...
        node that is the quote {best} break, so we must be careful to handle this
...
void begin_box(int box_context)
{
    ...
    /*tex a box number */
    int n;
    int spec_direction = direction_unknown;
    int just_pack = 0;
            ...
            if (abs(cur_list.mode_field) == mmode) {
                ...
                if (cur_list.head_field != cur_list.tail_field) {
                    if ((type(cur_list.tail_field) == hlist_node) || (type(cur_list.tail_field) == vlist_node)) {
                        ...
                        halfword q = alink(cur_list.tail_field);
