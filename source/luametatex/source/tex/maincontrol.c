...
/*tex
    ...
    We come now to the |main_control| routine, which contains the master switch
    that causes all the various pieces of \TEX\ to do their things, in the right
    order.

    In a sense, this is the grand climax of the program: It applies all the tools
    that we have worked so hard to construct. In another sense, this is the
    messiest part of the program: It necessarily refers to other pieces of code
    all over the place, so that a person can't fully understand what is going on
    without paging back and forth to be reminded of conventions that are defined
    elsewhere. We are now at the hub of the web, the central nervous system that
    ...
    called |big_switch|, at which point the next token of input is fetched using
    |get_x_token|. Then the program branches at high speed into one of about 100
    possible directions, based on the value of the current mode and the newly
    fetched command code; the sum |abs(mode) + cur_cmd| indicates what to do
    next. For example, the case |vmode+letter| arises when a letter occurs in
    vertical mode (or internal vertical mode); this case leads to instructions
    that initialize a new paragraph and enter horizontal mode.
    ...
    already been fetched. Most of the cases are quite short; they call an \quote
    {action procedure} that does the work for that case, and then they either
    |goto reswitch| or they \quote {fall through} to the end of the |case|
    ...
    and kerns, is part of \TEX's \quote {inner loop}; the whole program runs
    efficiently when its inner loop is fast, so this part has been written with
    particular care.

    We leave the |space_factor| unchanged if |sf_code(cur_chr) = 0|; otherwise we
    set it equal to |sf_code(cur_chr)|, except that it should never change from a
    value less than 1000 to a value exceeding 1000. The most common case is
...
static void insert_dollar_sign_par_end(void);
static void fixup_directions(void);
static void fixup_directions_only(void);
static void unpackage(void);
static int privileged(void);
static void new_interaction(void);
static void shift_case(void);
static void do_register_command(int a);
...
/*tex
    ...
    To handle the execution state of |main_control|'s eternal loop, an extra
    global variable is used, along with a macro to define its values.
...
/* what is the best state for these */
...
/*tex
    ...
    Here are all the functions that are called from |main_control| that are not
    already defined elsewhere. For the moment, this list simply in the order that
    the appear in |init_main_control|, below.
...
static void run_char_num(void) {
    ...
    cur_chr = cur_val;
    adjust_space_factor();
    tail_append(new_char(cur_font_par, cur_chr, character_data_par));
}
...
/*tex
    ...
    usually encounter about one space for every five non-blank characters.
...
/*tex Append a |boundary_node|. */

static void run_boundary(void) {
    halfword n = new_node(boundary_node,cur_chr);
...
static void run_char_ghost(void) {
    ...
    if ((cur_cmd == letter_cmd) || (cur_cmd == other_char_cmd)
        ...
        halfword p = new_glyph(cur_font_par, cur_chr, character_data_par);
        if (t == 0) {
            set_is_leftghost(p);
        } else {
            set_is_rightghost(p);
...
/*tex
    ...
    |ignore_spaces| is a special case: after it has acted, |get_x_token| has
    already fetched the next token from the input, so that operation in
    |main_control| should be skipped.
...
/*tex
    ...
    |stop| is the second special case. We want |main_control| to return to its
    caller if there is nothing left to do.
...
/*tex We build up an argument to |set_math_char|: */
...
/*tex
    ...
    vlists and hlists, as well as the activities that convert those lists into
    boxes. All of the necessary machinery has already been developed; it remains
    for us to \quote {push the buttons} at the right times.

    As an introduction to these routines, let's consider one of the simplest
    cases: What happens when |\hrule| occurs in vertical mode, or |\vrule| in
    horizontal mode or math mode? The code in |main_control| is short, since the
    |scan_rule_spec| routine already does most of what is required; thus, there
    is no need for a special action procedure.
...
/*tex
    ...
    braces in the input. For example, when the user says |\hbox to 100pt
    {<hlist>}| in vertical mode, the information about the box size (100pt,
    |exactly|) is put onto |save_stack| with a level boundary word just above it,
    and |cur_group:=adjusted_hbox_group|; \TEX\ enters restricted horizontal mode
    to process the hlist. The right brace eventually causes |save_stack| to be
    restored to its former state, at which time the information about the box
    size (100pt, |exactly|) is available once again; a box is packaged and we
    leave restricted horizontal mode, appending the new box to the current list
    of the enclosing mode (in this case to the current list of vertical mode),
    followed by any vertical adjustments that were removed from the box by
    |hpack|.
    ...
    introduces a new level of grouping, and the matching right brace will not
    have such a drastic effect. Such grouping affects neither the mode nor the
    current list.
...
/*tex
    ...
    specified context code. The |scan_box| routine verifies that a |make_box|
    command comes next and then it calls |begin_box|.

*/

static void scan_box(int box_context, int optional_equal);
...
static void run_leader_ship(void) {
    scan_box(leader_flag - a_leaders + cur_chr,0);
}
...
static void run_box_direction(void) {
    ...
    switch (t) {
        case 0:
            scan_int(1);
            box_dir(cur_box) = (quarterword) checked_direction_value(cur_val);
            break;
        case 1:
            scan_int(1);
            box_orientation(cur_box) = (quarterword) checked_orientation_value(cur_val);
            check_box_offsets(cur_box);
            break;
        case 2:
            scan_normal_dimen(1);
            box_x_offset(cur_box) = cur_val;
            check_box_offsets(cur_box);
            break;
        case 3:
            scan_normal_dimen(1);
            box_y_offset(cur_box) = cur_val;
            check_box_offsets(cur_box);
            break;
        case 4:
            {
                halfword offset, width;
                scan_normal_dimen(1);
                check_dimen1(offset, box_x_offset(cur_box) + cur_val);
                check_dimen2(width, width(cur_box) + cur_val);
                box_x_offset(cur_box) = offset;
                width(cur_box) = width;
                check_box_offsets(cur_box);
            }
            break;
        case 5:
            {
                halfword offset, height, depth;
                scan_normal_dimen(1);
                check_dimen1(offset, box_y_offset(cur_box) + cur_val);
                check_dimen2(height, height(cur_box) + cur_val);
                check_dimen2(depth, depth(cur_box) - cur_val);
                box_y_offset(cur_box) = offset;
...
/*tex
    ...
    vertical modes, it is identical to |\indent|, but in horizontal and math
    modes it is really a no|-|op (as opposed to |\indent|, which executes the
    |indent_in_hmode| procedure).

    A paragraph begins when horizontal-mode material occurs in vertical mode, or
    when the paragraph is explicitly started by |\quitvmode|, |\indent| or
    |\noindent|.
...
/*tex
    ...
    horizontal mode when reaching the right brace of vertical-mode routines like
    |\vbox|, |\insert|, or |\output|.
...
/*tex
    ...
    |after_token|. This global variable is examined just after every assignment
    has been performed. It's value is zero, or a saved token.
...
static void run_catcode_table(void) {
{
    switch (cur_chr) {
...
static void run_end_local(void) {
...
/*tex
    ...
    For mode-independent commands, the following macro is useful. Also, there is
    a list of cases where the user has probably gotten into or out of math mode
    by mistake. \TEX\ will insert a dollar sign and rescan the current token, and
...
static void init_main_control(void) {
    jump_table = malloc((mmode+max_command_cmd+1) * sizeof(main_control_function)) ;
    ...
    any_mode(catcode_table_cmd, run_catcode_table);
    any_mode(end_local_cmd, run_end_local);
...
/*tex And here is |main_control| itself. It is quite short nowadays. */
...
void main_control(void)
{
    ...
    while (1) {
        ...
        /*tex
            ...
            \TEX's activity.
        */
        if (error_state.interrupt != 0 && error_state.OK_to_interrupt) {
            back_input();
            check_interrupt();
            continue;
        } else if (tracing_commands_par > 0) {
            show_cur_cmd_chr();
        }
        /*tex run the command */
...
/*tex
    ...
    We assume a trailing |\relax|: |{...}\relax|, so we don't need a
    |back_input()| here.
...
/*tex handle spaces when |space_factor<>1000| */
...
/*tex
    ...
    specific to the particular error. For example, |\noalign| should not appear
    in any mode, since it is recognized by the |align_peek| routine in all of its
    legitimate appearances; a special error message is given when |\noalign|
    occurs elsewhere. But sometimes the most appropriate error message is simply
    that the user is not allowed to do what he or she has attempted. For example,
...
/*tex
    ...
    |mode > 0|. The |privileged| function is used to detect violations of this
    rule; it issues an error message and returns |false| if the current |mode| is
    negative.
...
/*tex
    ...
    times before things really grind to a halt. (The output routine might even
    say |\gdef \end {...}|, to prolong the life of the job.) Therefore
    |its_all_over| is |true| only when the current page and contribution list are
    empty, and when the last output was not a \quote {dead cycle}.

*/

/*tex Do this when |\end| or |\dump| occurs: */

int its_all_over(void)
{
    if (privileged()) {
        if ((page_head == page_tail) && (head == tail) && (main_state.dead_cycles == 0)) {
            return 1;
        }
...
/*tex
    ...
    |\hss| and |\vfil| as well as for |\hskip| and |\vskip|. The difference is in
    the value of |cur_chr|.
...
static void append_glue(void)
{
    ...
    switch (s) {
        ...
        case fil_neg_code:
            ...
            break;
        case skip_code:
            scan_glue(glue_val_level,0);
            break;
        case mskip_code:
            scan_glue(mu_val_level,0);
            break;
    }
...
/*tex
    ...
    tell one from the other, so it makes a guess and tries to avoid getting into
    a loop.

    The |off_save| routine is called when the current group code is wrong. It
    tries to insert something into the user's input that will help clean off the
    top level.
...
/*tex
    ...
    The routine for a |right_brace| character branches into many subcases, since
    a variety of things may happen, depending on |cur_group|. Some types of
    groups are not supposed to be ended by a right brace; error messages are
    given in hopes of pinpointing the problem. Most branches of this routine will
    be filled in later, when we are ready to understand them; meanwhile, we must
    prepare ourselves to deal with such errors.

*/

static void handle_right_brace(void)
{
    /*tex This one is needed for math group management: */
    halfword p = null;
    switch (cur_group) {
        case simple_group:
            fixup_directions();
        ...
        /*tex
            ...
            |\vtop| construction, the |package| routine comes into action. We
            might also have to finish a paragraph that hasn't ended.
        */
        case hbox_group:
            if (fixup_boxes_par) {
        ...
        case insert_group:
            {
                halfword q; /*tex for short-term use */
                scaled d;   /*tex holds |split_max_depth| in |insert_group| */
                int f;      /*tex holds |floating_penalty| in |insert_group| */
                ...
                */
                p = vpack(vlink(head), 0, additional, direction_unknown);
                pop_nest();
...
/*tex
    ...
    The global variable |cur_box| will point to a newly-made box. If the box is
    void, we will have |cur_box = null|. Otherwise we will have |type(cur_box) =
    hlist_node| or |vlist_node| or |rule_node|; the |rule_node| case can occur
    only with leaders.
...
/*tex The next input should specify a box or perhaps a rule: */
...
void new_graf(int indented)
{
    ...
    indented = lua_newgraf_callback(mode==vmode,indented);
    prev_graf_par = 0;
    push_nest();
    mode = hmode;
    space_factor_par = 1000;
    ...
    if (indented) {
        halfword p = new_null_box();
        box_dir(p) = (quarterword) par_direction_par;
    ...
    while (dir_rover) {
        if ((vlink(dir_rover)) || (dir_dir(dir_rover) != par_direction_par)) {
            halfword dir_graf_tmp = new_dir(normal_dir,dir_dir(dir_rover));
...
/*tex
    ...
    |dir_save| would have been set by |line_break| by means of |post_line_break|,
    but this is not done right now, as it introduces pretty heavy memory leaks.
    This means the current code might be wrong in some way that relates to
    in-paragraph displays.
...
void end_graf(int line_break_context)
{
    if (mode == hmode) {
        if ((head == tail) || (vlink(head) == tail)) {
            ...
            line_break(0, line_break_context);
        }
        if (dir_save != null) {
...
/*tex
    ...
    The |make_mark| procedure has been renamed, because if the current chr code
    is 1, then the actual command was |\clearmarks|, which does not generate a
    mark node but instead destroys the current mark tokenlists.

*/

static void handle_mark(void)
{
    halfword p; /*tex new node */
    halfword c; /*tex the mark class */
...
/*tex
    ...
    The |remove_item| command removes a penalty, kern, or glue node if it appears
    at the tail of the current list, using a brute-force linear scan. Like
...
static void unpackage(void)
{
    halfword p; /*tex the box */
    halfword r; /*tex to remove marginal kern nodes */
    int c;      /*tex should we copy? */
    halfword s; /*tex for varmem assignment */
...
/*tex
    ...
    follows a character. In math mode the same effect is achieved by appending a
    kern of zero here, since italic corrections are supplied later.

*/

static void append_italic_correction(void)
{
    if (tail != head && is_char_node(tail)) {
        halfword p = tail; /*tex |char_node| at the tail of the current list */
        tail_append(new_kern(char_italic_from_font(font(p), character(p))));
...
/*tex
    ...
    Discretionary nodes are easy in the common case |\-|, but in the general case
    we must process three braces full of items.
...
static void append_discretionary(void)
{
    ...
    if (cur_chr == explicit_disc) {
        /* |\-| */
        int c = get_pre_hyphen_char(cur_lang_par);
        if (c > 0) {
            vlink(pre_break(tail)) = new_char(cur_font_par, c, character_data_par);
            alink(vlink(pre_break(tail))) = pre_break(tail);
            tlink(pre_break(tail)) = vlink(pre_break(tail));
        }
        c = get_post_hyphen_char(cur_lang_par);
        if (c > 0) {
            vlink(post_break(tail)) = new_char(cur_font_par, c, character_data_par);
        ...
        int c = get_pre_exhyphen_char(cur_lang_par);
        if (c <= 0) {
        ...
        c = ex_hyphen_char_par;
        if (c > 0) {
            vlink(no_break(tail)) = new_char(cur_font_par, c, character_data_par);
...
static void build_local_box(void)
{
    ...
    int kind;
    unsave();
    if (saved_type(-1) != saved_boxtype) {
        confusion("build_local_box");
    }
    kind = saved_value(-1);
    decr(save_ptr);
    p = vlink(head);
    ...
    if (p) {
        ...
        */
        ...
        p = lua_hpack_filter(p, 0, additional, local_box_group, direction_unknown, null);
        /*tex
        ...
        */
        p = hpack(p, 0, additional, direction_unknown);
    }
    if (kind == 0)
...
static void build_discretionary(void)
{
    ...
    */
    ...
    p = vlink(head);
    pop_nest();
    if (saved_type(-1) != saved_disc) {
        confusion("build_discretionary");
    }
...
/*tex
    ...
    character of width |w|, height |h|, and slant |t|: We will shift the accent
    down by |x - h|, and we will insert kern nodes that have the effect of
    centering the accent over the character and shifting the accent to the right
    by $\delta = {1 \over 2} (w-a) + h \cdot t-x \cdot s$. If either character is
...
static void make_accent(void)
{
    ...
    halfword p; /* character */
    scan_char_num();
    f = cur_font_par;
    p = new_glyph(f, cur_val, character_data_par);
    if (p) {
        /*tex
        ...
        */
        halfword q = null;
        scaled x = x_height(f);
        double s = float_cast(slant(f)) / float_constant(65536);
        scaled a = glyph_width(p);
        do_assignments();
        f = cur_font_par;
        if ((cur_cmd == letter_cmd) ||
            (cur_cmd == other_char_cmd) || (cur_cmd == char_given_cmd)) {
            q = new_glyph(f, cur_chr, character_data_par);
        } else if (cur_cmd == char_num_cmd) {
            scan_char_num();
            q = new_glyph(f, cur_val, character_data_par);
        } else {
            back_input();
        }
        if (q) {
            /*tex
                ...
                |\lastkern|.
             */
            scaled delta; /* heights and widths, as explained above */
            halfword r; /* kern */
            double t = float_cast(slant(f)) / float_constant(65536); /* amount of slant */
            scaled w = glyph_width(q);
            scaled h = glyph_height(q);
            if (h != x) {
...
/*tex
    ...
    |main_control|, it might be that the user has foolishly inserted one of them
    into something that has nothing to do with alignment. But it is far more
    likely that a left brace or right brace has been omitted, since |get_next|
    takes actions appropriate to alignment only when |\cr| or |\span| or tab
    marks occur with |align_state=0|. The following program attempts to make an
...
/*tex
    ...
    We've now covered most of the abuses of |\halign| and |\valign|. Let's take a
    look at what happens when they are used correctly.

    An |align_group| code is supposed to remain on the |save_stack| during an
    entire alignment, until |fin_align| removes it.
...
/*tex
    ...
    control sequence can be defined to be |\long|, |\protected|, or |\outer|, and
    it might or might not be expanded. The prefixes |\global|, |\long|,
    |\protected|, and |\outer| can occur in any order. Therefore we assign binary
    numeric codes, making it possible to accumulate the union of all specified
    prefixes by adding the corresponding codes. (\PASCAL's |set| operations could
    also have been used.)

    Every prefix, and every command code that might or might not be prefixed,
    calls the action procedure |prefixed_command|. This routine accumulates a
    sequence of prefixes until coming to a non-prefix, then it carries out the
    command.
...
void inject_text_or_line_dir(int val, int check_glue)
{
    if (abs(mode) == hmode) {
        if (no_local_dirs_par > 0) {
            ...
            if (check_glue && (tail != null && type(tail) == glue_node))  {
                ...
                halfword dirn = new_dir(cancel_dir,text_direction_par);
                couple_nodes(prev,dirn);
                couple_nodes(dirn,tail);
            } else {
                tail_append(new_dir(cancel_dir,text_direction_par));
            }
        }
        update_text_dir_ptr(val);
        tail_append(new_dir(normal_dir,val));
...
{
    int a = 0;              /*tex accumulated prefix codes so far */
    internal_font_number f; /*tex identifies a font */
    halfword j;             /*tex index into a |\parshape| specification */
    halfword p, q;          /*tex for temporary short-term use */
    int n;                  /*tex ditto */
    int e, check_glue;      /*tex should a definition be expanded? or was |\let| not done? */
    mathcodeval mval;       /*tex for handling of |\mathchardef|s */
        ...
        */
        ...
        if (cur_cmd <= max_non_prefixed_command) {
            ...
            */
            ...
            back_error();
            return;
        } else if (tracing_commands_par > 2) {
            show_cur_cmd_chr();
        }
    }
    /*tex
    ...
    switch (cur_cmd) {
        ...
        case shorthand_def_cmd:
            /*tex
                ...
                instead of producing an \quote {undefined control sequence} error
                or expanding the previous meaning. This allows, for instance,
                |\chardef\foo=123\foo|.
            */
            n = cur_chr;
            get_r_token();
        ...
        case assign_toks_cmd:
           ...
           /*tex Skip an optional equal sign and get the next non-blank non-relax non-call token */
            /*
            scan_optional_equals();
            do {
                get_x_token();
            ...
            */
            ...
            if (cur_cmd != left_brace_cmd) {
                ...
                */
                ...
                /*tex So we can get an error message here. */
                back_input();
                cur_cs = q;
                q = scan_toks(0, 0, 0);
            } else {
            ...
            if (token_link(def_ref) == null) {
                /*tex empty list: revert to the default */
                define(p, undefined_cs_cmd, null);
                free_avail(def_ref);
            } else {
                if (p == output_routine_loc) {
                    /*tex enclose in curlies */
                    fast_get_avail(p);
                    set_token_link(q, p);
                ...
                case int_base + line_direction_code:
                    /*
                        ...
                        in the output when we use
                            ...
                            tail_append(new_dir(normal_dir,text_direction_par)
                    ...
                    */
                    ...
                    inject_text_or_line_dir(cur_val,check_glue);
        ...
        case assign_mu_glue_cmd:
            ...
            if (n == assign_mu_glue_cmd)
                scan_glue(mu_val_level,1);
            else
                scan_glue(glue_val_level,1);
        ...
        case set_box_cmd:
            ...
            */
            ...
            if (is_global(a))
                ...
                n = box_flag + cur_val;
            if (error_state.set_box_allowed) {
                scan_box(n,1);
            } else {
        ...
        case set_tex_shape_cmd:
            ...
            if (n <= 0) {
                ...
                for (j = 1; j <= n; j++) {
                    scan_normal_dimen(0);
                    vvalue(p + 2 * j) = cur_val;       /*tex indentation */
                    scan_normal_dimen(0);
                    vvalue(p + 2 * j + 1) = cur_val;   /*tex width */
                }
            }
        ...
        case set_etex_shape_cmd:
            ...
            if (n <= 0) {
                ...
                vinfo(p + 1) = n;
                n = cur_val;
                vvalue(p + 2) = n;             /*tex number of penalties */
                for (j = p + 3; j <= p + n + 2; j++) {
                    scan_int(0);
                    vvalue(j) = cur_val;       /*tex penalty values */
                }
                if (!odd(n))
                    vvalue(p + n + 3) = 0;     /*tex unused */
            }
            define(q, shape_ref_cmd, p);
            break;
        case hyph_data_cmd:
        ...
        case assign_font_int_cmd:
            ...
            if (n < lp_code_base) {
                ...
                switch (n) {
                    case lp_code_base:
                        set_lpcode_in_font(f, p, cur_val);
                        break;
                    case rp_code_base:
                        set_rpcode_in_font(f, p, cur_val);
                        break;
                    case ef_code_base:
                        set_efcode_in_font(f, p, cur_val);
                        break;
                }
            }
            break;
...
/*tex This is experimental and needs more checking! */
...
static void fixup_directions_only(void)
{
    ...
    if (temp_no_dirs != 0) {
        /* Add local dir node. */
        tail_append(new_dir(cancel_dir,temporary_dir));
    }
}
...
/*tex
    ...
    similar, the |get_r_token| routine will substitute a special control sequence
    for a token that is not redefinable.
...
void assign_internal_value(int a, halfword p, int val)
{
    if ((p >= int_base) && (p < attribute_base)) {
        ...
        /*tex
            ...
            step.
        */
        if ((abs(mode) == hmode) && ((p == (int_base + local_inter_line_penalty_code)) || (p == (int_base + local_broken_penalty_code)))) {
            /*tex Add local paragraph node */
  ...
  FOUND:
    ...
    if (q < multiply_cmd) {
        ...
        if (p < glue_val_level) {
            if (q == multiply_cmd) {
                if ((p == int_val_level) || (p == attr_val_level)) {
                    cur_val = mult_integers(equiv_value(l), cur_val);
                } else {
                    cur_val = nx_plus_y(equiv_value(l), cur_val, 0);
                }
            } else {
                cur_val = x_over_n(equiv_value(l), cur_val);
            }
        } else {
            halfword s = equiv(l);
...
/*tex The value of |c| is 0 for |\deadcycles|, 1 for |\insertpenalties|, etc. */
...
static void alter_integer(void)
{
    ...
    scan_int(1);
    if (c == 0) {
        main_state.dead_cycles = cur_val;
    } else if (c == 2) {
...
static void alter_box_dimen(void)
{
    int b; /*tex box number */
    int c = cur_chr; /*tex |width_offset| or |height_offset| or |depth_offset| */
    scan_register_num();
    b = cur_val;
    scan_normal_dimen(1);
    if (box(b) != null)
        vvalue(box(b) + c) = cur_val;
}

static void new_interaction(void)
{
    print_ln();
    error_state.interaction = cur_chr;
    fixup_selector(fileio_state.log_opened);
}
...
static void open_or_close_in(void)
{
    ...
    if (c != 0) {
        char *fn = read_file_name(1,NULL,texinput_extension);
...
static int long_help_seen;

static void issue_message(void)
{
    int old_setting; /*tex holds |selector| setting */
    int c;           /*tex identifies |\message| and |\errmessage| */
    str_number s;    /*tex the message */
    c = cur_chr;
    scan_toks(0, 1, 0);
    ...
    if (c == 0) {
        ...
        */
        ...
        if (err_help_par != null) {
            error_state.use_err_help = 1; /*tex never reset */
...
static void shift_case(void)
{
    halfword c; /*tex character code */
    halfword i; /*tex inbetween */
    halfword b = cur_chr; /*tex |lc_code_base| or |uc_code_base| */
    halfword p = scan_toks(0, 0, 0); /*tex runs through the token list */
    ...
    while (p != null) {
        ...
        if (t < cs_token_flag) {
            ...
            if (b == uc_code_base)
                ...
                i = get_lc_code(c);
            if (i != 0)
                set_token_info(p, active_to_cs(i, 1) + cs_token_flag);
        }
        p = token_link(p);
    }
...
static void show_whatever(void)
{
    halfword p; /*tex tail of a token list to show */
    int t;      /*tex type of conditional being shown */
    int m;      /*tex upper bound on |fi_or_else| codes */
    int l;      /*tex line where that conditional began */
    int n;      /*tex level of |\if ... \fi| nesting */
    switch (cur_chr) {
        case show_lists:
            begin_diagnostic();
            show_activities();
...
void initialize(void)
{
    ...
    */
    ...
    no_new_control_sequence = 1; /*tex new identifiers are usually forbidden */
    init_primitives();

    initialize_marks();
    initialize_read();

    dump_state.format_ident = 0;
    dump_state.format_name = get_nullstr();
    ...
    if (main_state.ini_version) {
        ...
        for (k = attribute_base; k <= del_code_base - 1; k++)
            equiv_value(k) = UNUSED_ATTRIBUTE;
        mag_par = 1000;
        tolerance_par = 10000;
        hang_after_par = 1;
        ...
        for (k = dimen_base; k <= eqtb_size; k++)
            equiv_value(k) = 0;
        ini_init_primitives();
        hash_state.hash_used = frozen_control_sequence;
        hash_state.hash_high = 0;
        hash_state.cs_count = 0;
        ...
        create_null_font();
        font_state.font_bytes = 0;
        cs_text(frozen_protection) = maketexstring("inaccessible");
        dump_state.format_ident = maketexstring(" (INITEX)");
