...
/*tex
    ...
    It's sort of a miracle whenever |halign and |valign| work, because they cut
    across so many of the control structures of \TeX. Therefore the present page
    is probably not the best place for a beginner to start reading this program;
    it is better to master everything else first.
    ...
    \startitemize
        ...
        \startitem
            When |\halign to 300pt {}| is scanned, the |scan_spec| routine places
            the 300pt dimension onto the |save_stack|, and an |align_group| code
            is placed above it. This will make it possible to complete the
            alignment when the matching right brace is found.
        \stopitem
        ...
        \startitem
            ...
            These \quote {alignrecord} entries have the same size as an |unset_node|,
            ...
            \type{#} for column~1. Furthermore, They have a |span_ptr| instead of a
            |node_attr| field, and these |span_ptr| fields are initially set to
            the value |end_span|, for reasons explained below.
        \stopitem

        \startitem
            \TEX\ now looks at what follows the |\cr| that ended the preamble. It
            is not |\noalign| or |\omit|, so this input is put back to be read
            again, and the template |u1| is fed to the scanner. Just before
            reading |u1|, \TeX\ goes into restricted horizontal mode. Just after
            reading |u1|, \TEX\ will see |a1|, and then (when the |&| is sensed)
            \TEX\ will see |v1|. Then \TEX\ scans an |endv| token, indicating the
            end of a column. At this point an |unset_node| is created, containing
            the contents of the current hlist (i.e., |u1a1v1|). The natural width
            of this unset node replaces the |width| field of the alignrecord for
            column~1; in general, the alignrecords will record the maximum
            natural width that has occurred so far in a given column.
        \stopitem

        \startitem
            Since |\omit| follows the |&|, the templates for column~2 are now
            bypassed. Again \TEX\ goes into restricted horizontal mode and makes
            an |unset_node| from the resulting hlist; but this time the hlist
            contains simply |a2|. The natural width of the new unset box is
        ...
        \startitem
            ...
            v3|. The vertical rule in this case has running dimensions that will
            later extend to the height and depth of the whole first row, since
            each |unset_node| in a row will eventually inherit the height and
            depth of its enclosing box.
        \stopitem
        ...
        \startitem
            Since |\noalign| follows the current |\cr|, \TEX\ appends additional
            material (in this case |\vskip 3pt|) to the vertical list. While
            processing this material, \TeX\ will be in internal vertical mode,
            and |no_align_group| will be on |save_stack|.
        \stopitem
        ...
        \startitem
            ...
            stored in a \quote {span node}, which we will explain later; the
...
/*tex
    ...
    |\cr| and |\span| and tab marks are properly intercepted. There also are
    pointers |cur_head| and |cur_tail| to the head and tail of a list of
...
typedef struct alignment_state_info {
    halfword cur_align; // null
    halfword cur_span;  // null
    halfword cur_loop;  // null
    halfword align_ptr; // null
    halfword cur_head;  // null
    halfword cur_tail;  // null
    halfword cur_pre_head; // null
    halfword cur_pre_tail; // null
} alignment_state_info ;

static alignment_state_info alignment_state;
...
# define cur_align alignment_state.cur_align

/*tex The start of the currently spanned columns in the preamble list: */

# define cur_span alignment_state.cur_span

/*tex A place to copy when extending a periodic preamble: */

# define cur_loop alignment_state.cur_loop

/*tex The most recently pushed-down alignment stack node: */

# define align_ptr alignment_state.align_ptr

/*tex Adjustment list pointers: */

# define cur_head alignment_state.cur_head
# define cur_tail alignment_state.cur_tail

/*tex Pre-adjustment list pointers: */

# define cur_pre_head alignment_state.cur_pre_head
# define cur_pre_tail alignment_state.cur_pre_tail
...
/*tex
    ...
    It makes not much sense to add support for an |attr| keyword to |\halign| and
    |\valign| because then we need to decide if we tag rows or cells or both or
    come up with |cellattr| and |rowattr| and such. But then it even makes sense
    to have explicit commands (in addition to the seperator) to tags individual
    cells. Too muss hassle for now and the advantages are not that large.
...
static void push_alignment(void)
{
    ...
    halfword p = new_node(align_stack_node, 0);
    vinfo(p + 1) = align_ptr;
    vlink(p + 1) = cur_align;
    vinfo(p + 2) = preamble;
    vlink(p + 2) = cur_span;
...
/*tex
    ...
    Note that if the preamble contains |\global\tabskip|, the |\global| token
    survives in the preamble and the |\tabskip| defines new tabskip glue
    (locally).
...
static void get_preamble_token(void)
{
  RESTART:
    ...
    if ((cur_cmd == assign_glue_cmd) && (cur_chr == glue_base + tab_skip_code)) {
...
/*tex
    ...
    When |\halign| or |\valign| has been scanned in an appropriate mode, \TEX\
    calls |init_align|, whose task is to get everything off to a good start. This
    mostly involves scanning the preamble and putting its information into the
    preamble list.
...
void init_align(void)
{
    ...
    */
    ...
    while (1) {
        ...
        halfword p;
        halfword r = new_param_glue(tab_skip_code);
        vlink(cur_align) = r;
...
/*tex
    ...
    We usually begin a row after each |\cr| has been sensed, unless that |\cr| is
    followed by |\noalign| or by the right brace that terminates the alignment.
    The |align_peek| routine is used to look ahead and do the right thing; it
    either gets a new row started, or gets a |\noalign} started, or finishes off
    the alignment.
...
static void init_span(halfword p)
{
    push_nest();
...
/*tex
    ...
    To start a row (i.e., a \quote {row} that rhymes with \quote {dough} but not
    with \quote {bough}), we enter a new semantic level, copy the first tabskip
    glue, and change from internal vertical mode to restricted horizontal mode or
    vice versa. The |space_factor| and |prev_depth| are not used on this semantic
    level, but we clear them to zero just to be tidy.
...
# define determine_order(total,o) do { \
    if      (total[filll]!= 0) o = filll; \
    else if (total[fill] != 0) o = fill; \
    else if (total[fil]  != 0) o = fil; \
    else if (total[sfi]  != 0) o = sfi; \
    else                       o = normal; \
} while (0)
...
static halfword new_span_node(halfword n, int s, scaled w)
{
...
int fin_col(void)
{
    ...
    if ((p == null) && (extra_info(cur_align) < cr_code)) {
        if (cur_loop != null) {
            ...
            halfword r = new_node(align_record_node, 0);
            vlink(q) = r;
            /*tex A new align record: */
            p = vlink(q);
            span_ptr(p) = end_span;
    ...
    if (extra_info(cur_align) != span_code) {
        ...
        if (cur_list.mode_field == -hmode) {
            ...
            u = filtered_hpack(cur_list.head_field, cur_list.tail_field, 0, additional, align_set_group, direction_unknown, 0, 0);
            w = width(u);
            ...
            u = filtered_vpackage(vlink(cur_list.head_field), 0, additional, 0, align_set_group, direction_unknown, 0, 0);
            w = height(u);
        }
        ...
        determine_order(total_stretch,o);
        glue_order(u) = o;
        glue_stretch(u) = total_stretch[o];
        determine_order(total_shrink,o);
        glue_sign(u) = o;
...
/*tex
    ...
    |\halign|) or the current hlist (for |\valign|). This unset box contains the
    unset boxes for the columns, separated by the tabskip glue. Everything will
    be set later.
...
void fin_row(void)
{
    ...
    halfword p;
    if (cur_list.mode_field == -hmode) {
        p = filtered_hpack(cur_list.head_field, cur_list.tail_field, 0, additional, fin_row_group, direction_unknown, 0, 0);
        pop_nest();
        ...
        p = filtered_vpackage(vlink(cur_list.head_field), 0, additional, max_depth_par, fin_row_group, direction_unknown, 0, 0);
        pop_nest();
...
static void fin_align(void)
{
    ...
    */
    ...
    /*tex
        ...
        column sizes. In case of |\valign|, we change the widths to heights, so
        that a correct error message will be produced if the alignment is
    ...
    */
    ...
    if (cur_list.mode_field == -vmode) {
        ...
        overfull_rule_par = 0;
        p = hpack(preamble, saved_value(0), saved_level(0), direction_unknown);
        ...
        p = filtered_vpackage(preamble, saved_value(0), saved_level(0), max_depth_par, preamble_group, direction_unknown, 0, 0);
        q = vlink(preamble);
        do {
            width(q) = height(q);
            height(q) = 0;
            ...
            if (type(q) == unset_node) {
                ...
                */
                ...
                if (o != 0) {
                    r = vlink(q);
                    vlink(q) = null;
                    q = hpack(q, 0, additional, direction_unknown);
                    shift_amount(q) = o;
