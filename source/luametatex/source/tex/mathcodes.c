...
static void unsavemathcode(quarterword gl)
{
    if (mathcode_head->stack == NULL) {
        ...
        while (mathcode_head->sa_stack_ptr > 0 && abs(mathcode_head->stack[mathcode_head->sa_stack_ptr].level) >= gl) {
            sa_stack_item st = mathcode_head->stack[mathcode_head->sa_stack_ptr];
            if (st.level > 0) {
...
static void unsavedelcode(quarterword gl)
{
    if (delcode_head->stack == NULL) {
        ...
        while (delcode_head->sa_stack_ptr > 0 && abs(delcode_head->stack[delcode_head->sa_stack_ptr].level) >= gl) {
            sa_stack_item st = delcode_head->stack[delcode_head->sa_stack_ptr];
            if (st.level > 0) {
