...
/*tex
    ...
    covered its important |try_break| subroutine. Now let's consider the rest of
    the process.

    The main loop of |line_break| traverses the given hlist, starting at
    |vlink(temp_head)|, and calls |try_break| at each legal breakpoint. A
    variable called |auto_breaking| is set to true except within math formulas,
    since glue nodes are not legal breakpoints when they appear in formulas.
    ...
    space as the |prev_break| fields did, but we give them a new name because of
    their new significance.) Then the lines are justified, one by one.

    The |post_line_break| must also keep an dir stack, so that it can output end
    direction instructions at the ends of lines and begin direction instructions
    ...
    */
    ...
    do {
        ...
        */
        if (dir_ptr != null) {
            ...
            for (q = dir_ptr; q != null; q = vlink(q)) {
                halfword tmp = new_dir(normal_dir,dir_dir(q));
        ...
        */
        ...
        if (have_directional) {
            ...
            for (p = dir_ptr; p != null; p = vlink(p)) {
                halfword s = new_dir(cancel_dir,dir_dir(p));
        ...
        */
        ...
        if (passive_left_box(cur_p) != null) {
            halfword s;
        ...
        */
        if (cur_line > last_special_line) {
            ...
            cur_indent = vvalue(par_shape_ptr + 2 * cur_line);
            cur_width = vvalue(par_shape_ptr + 2 * cur_line + 1);
            /* needs checking */
