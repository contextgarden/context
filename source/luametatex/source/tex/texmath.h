...
extern halfword new_style(halfword);
...
extern void unsave_math_data(int lvl);

/*tex

    We also need to compute the change in style between mlists and their
    subsidiaries. The following macros define the subsidiary style for an
    overlined nucleus (|cramped_style|), for a subscript or a superscript
...
/*tex Add this to an uncramped style if you want to cramp it: */
...
# define num_style(A)     (A)+2-2*((A)/6)                 /*tex smaller unless already script-script */
...
typedef enum {
    ...
    math_param_nolimit_sub_factor, /*tex bonus */
    math_param_nolimit_sup_factor, /*tex bonus */
