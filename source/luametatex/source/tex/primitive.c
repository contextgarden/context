...
/*tex
    ...
    table algorithm called the method of \quote {coalescing lists} (cf.\
    Algorithm 6.4C in {\em The Art of Computer Programming}). Once a control
    sequence enters the table, it is never removed, because there are complicated
    situations involving |\gdef| where the removal of a control sequence at the
    end of a group would be a mistake preventable only by the introduction of a
    ...
    auxiliary array |hash| consists of items with two halfword fields per word.
    The first of these, called |next(p)|, points to the next identifier belonging
    to the same coalesced list as the identifier corresponding to~|p|; and the
    other, called |text(p)|, points to the |str_start| entry for |p|'s
    identifier. If position~|p| of the hash table is empty, we have |text(p)=0|;
    if position |p| is either empty or the end of a coalesced hash list, we have
    |next(p) = 0|. An auxiliary pointer variable called |hash_used| is maintained
    in such a way that all locations |p >= hash_used| are nonempty. The global
    variable |cs_count| tells how many multiletter control sequences have been
    defined, if statistics are being kept.

    A boolean variable called |no_new_control_sequence| is set to |true| during
    the time that new hash table entries are forbidden.

    The other variables in the following state structure are: the hash table:
    |hash|, the allocation pointer |hash_used| for |hash|, |hash_extra| above
    |eqtb_size|, the maximum of the hash array |hash_top|, the pointer to the
    next high hash location |hash_high|, the mentioned flag that says if new
    identifiers are legal |no_new_control_sequence| and the total number of known
    identifiers: |cs_count|.

*/

hash_state_info hash_state = { NULL };

/*tex Test if all positions are occupied: */

# define hash_is_full (hash_state.hash_used==hash_base)
...
/*tex
    ...
    The arrays |prim| and |prim_eqtb| are used for name -> cmd,chr lookups. The
    are modelled after |hash| and |eqtb|, except that primitives do not have an
    ...
    \starttyping
    # define prim_text(a) prim[(a)].rhfield
    \stoptyping
    ...
    The array |prim_data| works the other way around, it is used for cmd,chr ->
    name lookups.

*/

prim_state_info prim_state;
...
/*tex
    ...
    should be a prime number. The theory of hashing tells us to expect fewer than
    two table probes, on the average, when the search is successful. [See J.~S.
    Vitter, {\sl Journal of the ACM\/ \bf30} (1983), 231--258.]

*/

inline static halfword compute_hash(const char *j, unsigned int l, halfword prime_number)
{
    int k;
...
halfword prim_lookup(str_number s)
{
    /*tex The index in the |hash| array: */
    if (s < STRING_OFFSET) {
        if ((s < 0) || (get_prim_eq_type(s) == undefined_cs_cmd)) {
        ...
        int h = compute_hash((char *) j, l, prim_prime);
        /*tex We start searching here; note that |0<=h<hash_prime|. */
        halfword p = h + prim_base;
        while (1) {
            if (prim_text(p) > 0 && str_length(prim_text(p)) == l && str_eq_str(prim_text(p), s)) {
                return p;
            } else if (prim_next(p) == 0) {
...
int is_primitive(str_number csname)
{
    int m = prim_lookup(csname);
    if (m != undefined_primitive) {
        char *ss = makecstring(csname);
        int n = string_lookup(ss, str_length(csname));
        free(ss);
        return ((n != undefined_cs_cmd) && (eq_type(n) == prim_eq_type(m)) && (equiv(n) == prim_equiv(m)));
    }
    return 0;
}
...
void undump_primitives(void)
{
    ...
    for (p = 0; p <= last_cmd; p++) {
        ...
        if (prim_data[p].subids > 0) {
            prim_data[p].names = (str_number *) malloc((unsigned) ((unsigned) prim_data[p].subids * sizeof(str_number *)));
            for (q = 0; q < prim_data[p].subids; q++) {
...
/*tex
    ...
    We need to put \TEX's \qtuote {primitive} control sequences into the hash
    table, together with their command code (which will be the |eq_type|) and an
    operand (which will be the |equiv|). The |primitive| procedure does this, in
    a way that no \TEX\ user can. The global value |cur_val| contains the new
    |eqtb| pointer after |primitive| has acted.

    Because the definitions of the actual user-accessible name of a primitive can
    be postponed until runtime, the function |primitive_def| is needed that does
    nothing except creating the control sequence name.
...
/*tex
    ...
    lookup. It is quite paranoid, because it is easy to mess this up
    accidentally.

    The |offset| is needed because sometimes character codes (in |o|) are indices
    into |eqtb| or are offset by a magical value to make sure they do not
    conflict with something else. We don't want the |prim_data[c].names| to have
    too many entries as it will just be wasted room, so |offset| is substracted
    from |o| because creating or accessing the array.
...
static void store_primitive_name(str_number s, quarterword c, halfword o, halfword offset)
{
    ...
    if (prim_data[c].subids < (idx + 1)) {
        str_number *new = (str_number *) calloc((unsigned) (idx + 1), sizeof(str_number *));
...
/*tex
    ...
    Compared to \TEX82, |primitive| has two extra parameters. The |off| is an
    offset that will be passed on to |store_primitive_name|, the |cmd_origin| is
    the bit that is used to group primitives by originator.
...
void primitive(const char *thes, quarterword c, halfword o, halfword off, int cmd_origin)
{
    ...
    int prim_val;
    str_number ss = maketexstring(thes);
...
/*tex
    ...
    Here is a helper that does the actual hash insertion. This code far from
    ideal: the existence of |hash_extra| changes all the potential (short)
    coalesced lists into a single (long) one. This will create a slowdown.
...
static halfword insert_id(halfword p, const unsigned char *j, unsigned int l)
{
    ...
    if (cs_text(p) > 0) {
        if (hash_state.hash_high < hash_state.hash_extra) {
            incr(hash_state.hash_high);
            /*tex
                Can't we use |eqtb_top| here (perhaps because that is not
                finalized yet when called from |primitive|?
            */
            cs_next(p) = hash_state.hash_high + eqtb_size;
            p = cs_next(p);
        } else {
            /*tex Search for an empty location in |hash|. */
            do {
                if (hash_is_full)
                    overflow("hash size", (unsigned) (hash_size + hash_state.hash_extra));
                decr(hash_state.hash_used);
            } while (cs_text(hash_state.hash_used) != 0);
            cs_next(p) = hash_state.hash_used;
            p = hash_state.hash_used;
        }
    }
...
/*tex
    ...
    matches a given string of length |l>1| appearing in |buffer[j.. (j+l-1)]|. If
    the identifier is found, the corresponding hash table address is returned.
    Otherwise, if the global variable |no_new_control_sequence| is |true|, the
    dummy address |undefined_control_sequence| is returned. Otherwise the
    identifier is inserted into the hash table and its location is returned.

*/

halfword id_lookup(int j, int l)
{
    /*tex The index in |hash| array: */
    int h = compute_hash((char *) (iobuffer + j), (unsigned) l, hash_prime);
    /*tex We start searching here. Note that |0<=h<hash_prime|: */
    halfword p = h + hash_base;
    /*tex The next one in a list: */
    halfword n;
    while (1) {
        str_number s = cs_text(p);
        if ((s > 0) && (str_length(s) == (unsigned) l)) {
            if (str_eq_buf(s,j,l)) {
                return p;
            }
        }
        ...
        if (n == 0) {
            if (no_new_control_sequence) {
                ...
                p = insert_id(p, (iobuffer + j), (unsigned) l);
            }
            return p;
        }
        p = n;
...
/*tex
    ...
    Here is a similar subroutine for finding a primitive in the hash. This one is
    based on a \CCODE\ string.

*/

halfword string_lookup(const char *s, size_t l)
{
    /*tex The hash code: */
    int h = compute_hash(s, (unsigned) l, hash_prime);
...
/*tex
    ...
    The |print_cmd_chr| routine prints a symbolic interpretation of a command
    code and its modifier. This is used in certain \quotation {You can\'t} error
    messages, and in the implementation of diagnostic routines like |\show|.
    ...
    Thanks to |prim_data|, there is no need for all that tediousness. What is
    left of |primt_cnd_chr| are just the exceptions to the general rule that the
