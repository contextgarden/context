...
/*tex
    ...
    calculations are done \quote {on line} as new items come in. The main
    complication in this process is that insertions must be put into their boxes
    and removed from the vlist, in a more-or-less optimum manner.

    We shall use the term \quote {current page} for that part of the main vlist
    that is being considered as a candidate for being broken off and sent to the
...
page_builder_state_info page_builder_state;

static void fire_up(halfword c);
...
/*tex
    ...
    split into two parts; the \quote {current page} that we have been talking so
    much about already, and the |quote {contribution list} that receives new
    nodes as they are created. The current page contains everything that the page
    builder has accounted for in its data structures, as described above, while
    the contribution list contains other things that have been generated by other
    parts of \TEX\ but have not yet been seen by the page builder. The
    contribution list starts at |vlink (contrib_head)|, and it ends at the
...
/*tex
    ...
    contribution list, if this was a glue node; otherwise |last_glue =
    max_halfword|. (If the contribution list is nonempty, however, the value of
    |last_glue| is not necessarily accurate.) The variables |last_penalty|,
    |last_kern|, and |last_node_type| are similar. And finally,
...
static void freeze_page_specs(int s)
{
    page_contents = s;
    page_goal = vsize_par;
...
/*tex
    ...
    routine is driving \TEX. The page builder is ready to start a fresh page if
    we initialize the following state variables. (However, the page insertion
    list is initialized elsewhere.)
...
/*tex
    ...
    At certain times box |\outputbox| is supposed to be void (i.e., |null|), or
    an insertion box is supposed to be ready to accept a vertical list. If not,
    an error message is printed, and the following subroutine flushes the
            ...
            case ins_node:
                {
                    ...
                    if (subtype(r) != n) {
                        /*tex
                            ...
                            Create a page insertion node with |subtype(r) = qi(n)|,
                            and include the glue correction for box |n| in the
                            current page state.

                            We take note of the value of |\skip| |n| and the height
                            plus depth of |\box| |n| only when the first |\insert|
                            |n| node is encountered for a new page. A user who
                            changes the contents of |\box| |n| after that first
                            |\insert| |n| had better be either extremely careful or
                            extremely lucky, or both.

                        */
                        int sk;
                        int id = callback_defined(build_page_insert_callback);
                        if (id != 0) {
                    ...
                    if (type(r) == split_up_node) {
                        ...
                        if (((h <= 0) || (h <= delta))
                            ...
                            /*tex
                                ...
                                total contents of box |n| greater than |\dimen| |n|,
                                or because it would make the incremental amount of
                                growth |h| greater than the available space |delta|,
                                or both. (This amount |h| has been weighted by the
                                insertion scaling factor, i.e., by |\count| |n| over
                                1000.) Now we will choose the best way to break the
                                vlist of the insertion, using the same criteria as in
...
/*tex
    ...
    badness will be used to put a completed page into box |\outputbox|, with
    insertions appended to their other boxes.

    We also set the values of |top_mark|, |first_mark|, and |bot_mark|. The
    program uses the fact that |bot_mark(x) <> null| implies |first_mark(x) <>
    null|; it also knows that |bot_mark(x) = null| implies |top_mark(x) =
    first_mark(x) = null|.
...
static void fire_up(halfword c)
{
                        ...
                        */
                        if (type(r) == split_up_node) {
                            if ((broken_ins(r) == p) && (broken_ptr(r) != null)) {
                                ...
                                ins_ptr(p) =
                                    prune_page_top(broken_ptr(r), 0);
                                if (ins_ptr(p) != null) {
                                    t = vpack(ins_ptr(p), 0, additional, direction_unknown);
                        ...
                        list_ptr(box(n)) = null;
                        flush_node(box(n));
                        box(n) = vpack(t, 0, additional, dir_lefttoright);
    ...
    */
    ...
    vfuzz_par = max_dimen;
    box(output_box_par) = filtered_vpackage(vlink(page_head), best_size, exactly, page_max_depth, output_group, dir_lefttoright, 0, 0);
    ...
    if (output_routine_par != null) {
        if (main_state.dead_cycles >= max_dead_cycles_par) {
            ...
            output_active = 1;
            incr(main_state.dead_cycles);
            push_nest();
            mode = -vmode;
            prev_depth_par = ignore_depth;
            mode_line_par = -input_line;
    ...
    */
    ...
    page_disc = null;
    if (box(output_box_par) != null) {
        flush_node_list(box(output_box_par));
