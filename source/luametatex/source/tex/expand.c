...
expand_state_info expand_state = { 0, 0, { 0 }, 0 };

static void macro_call(void);
static void insert_relax(void);
static void manufacture_csname(int use);
static void inject_last_tested_cs(void);
...
/*tex
    ...
    \quote {call} or a conditional or one of the other special operations just
    listed. It follows that |expand| might invoke itself recursively. In all
    cases, |expand| destroys the current token, but it sets things up so that the
    next |get_next| will deliver the appropriate next token. The value of
...
void expand(void)
{
    incr(expand_state.expand_depth_count);
    if (expand_state.expand_depth_count >= main_state.expand_depth) {
        overflow("expansion depth", (unsigned) main_state.expand_depth);
    } else {
        int cv_backup = cur_val;
        int cvl_backup = cur_val_level;
        int radix_backup = cur_radix;
        int co_backup = cur_order;
        halfword backup_backup = token_link(backup_head);
      RESWITCH:
        if (cur_cmd < call_cmd) {
            ...
            switch (cur_cmd) {
                            ...
                            case 3:
                                {
                                    /*tex
                                        ...
                                        This is an experiment: |\futureexpand| (2) which takes
                                        |\check \yes \nop| as arguments. It's not faster, but
                                        gives less tracing noise than a macro. The variant
                                        |\futureexpandis| (3) alternative doesn't inject the
                                        gobbles space(s).
                                    ...
                                    */
                                    ...
                                    cmd = cur_cmd;
                                    get_token();
                                    yes = cur_tok; /* when match */
                                    get_token();
                                    nop = cur_tok; /* when no match */
                                    while (1) {
                                        get_token();
                                    ...
                                    if (cur_cmd == cmd && cur_chr == chr) {
                                        reinsert_token(1,yes);
                                    } else {
                                        if (spa && mode == 2) {
                                            reinsert_token(1,space_token);
                                        }
                                        reinsert_token(1,nop);
                                    }
                                }
                                break;
                        }
                 ...
                 // if (cur_chr == 0) {
                    {
                        /*tex
                            ...
                            of |\noexpand| is a bit trickier, because it is necessary
                            to insert a special |dont_expand| marker into \TEX's
                            reading mechanism. This special marker is processed by
                            |get_next|, but it does not slow down the inner loop.
                        ...
                        */
                        halfword t;
                        halfword save_scanner_status = scanner_status;
                ...
                // } else {
                 ...
                 //
                 //         This is a rather useless primitive. We now can easily
                 //         create a set of aliases anyway so we can as well remove
                 //         this one.
                 //
                 //     */
                 //     halfword t;
                ...
                case if_test_cmd:
                    /*tex
                        ...
                        The |\ifcondition| primitive is a no-op unless we're in
                        skipping mode.
                ...
                case input_cmd:
                    ...
                    if (cur_chr == 1)
                        ...
                        pseudo_start();
                        iname = 19;
                    } else if (fileio_state.name_in_progress)
                        insert_relax();
                    else
                        start_input();
                    break;
        ...
        cur_radix = radix_backup;
        cur_order = co_backup;
        set_token_link(backup_head, backup_backup);
    }
    decr(expand_state.expand_depth_count);
}
...
static void manufacture_csname(int use)
{
 ...
 /* ss = tokenlist_to_cstring(r, 1, &siz); */
    ss = tokenlist_to_tstring(r, 1, &siz);

    expand_state.is_in_csname -= 1;
    if (use) {
        if (siz > 0) {
            cur_cs = string_lookup((char *) ss, siz);
        } else {
            cur_cs = null_cs;
        }
        last_cs_name = cur_cs ;
...
static void macro_call(void)
{
    ...
    if (token_info(r) != end_match_token) {
        ...
        */
        ...
        unbalance = 0;
        expand_state.long_state = eq_type(cur_cs);
        if (expand_state.long_state >= outer_call_cmd)
            expand_state.long_state = expand_state.long_state - 2;
        do {
          ...
          CONTINUE:
            ...
            if (cur_tok == token_info(r)) {
                /*tex
                    ...
                    delimiter ends with |#|, the token list will have a left
                    brace both before and after the |end_match|. Only one of
                    these should affect the |align_state|, but both will be
                    scanned, so we must make a correction.
            ...
            /*tex
                ...
                certain parameters of non-|\long| macros. For example, consider a
                case like |\def\a#1\par!{...}| where the first |\par| is not
                followed by an exclamation point. In such situations it does not
                seem appropriate to prohibit the |\par|, so \TEX\ keeps quiet
                about this bending of the rules.
            ...
            */
            ...
            if (cur_tok == par_token)
                if (expand_state.long_state != long_call_cmd)
            ...
            if (cur_tok < right_brace_limit) {
                if (cur_tok < left_brace_limit) {
                    ...
                    incr(align_state);
                    expand_state.long_state = call_cmd;
                    cur_tok = par_token;
                    ins_error();
                    goto CONTINUE;
            ...
            if (s != null) {
                ...
                */
                if ((m == 1) && (token_info(p) < right_brace_limit)
                    ...
                    expand_state.pstack[n] = token_link(p);
                    free_avail(p);
                } else {
                    expand_state.pstack[n] = token_link(temp_token_head);
                }
                incr(n);
                if (tracing_macros_par > 0) {
                    begin_diagnostic();
                    ...
                    print_int(n);
                    tprint("<-");
                    show_token_list(expand_state.pstack[n - 1], null, 1000);
                    end_diagnostic(0);
                }
            }
            /*tex
    ...
    */
    ...
    if (n > 0) {
        if (param_ptr + n > max_param_stack) {
            ...
            if (max_param_stack > main_state.param_size)
                overflow("parameter stack size", (unsigned) main_state.param_size);
        }
        for (m = 0; m <= n - 1; m++)
            param_stack[param_ptr + m] = expand_state.pstack[m];
  ...
  RUNAWAY:
    ...
    */
    if (expand_state.long_state == call_cmd) {
        runaway();
    ...
    for (m = 0; m <= n; m++)
        flush_list(expand_state.pstack[m]);
  EXIT:
    scanner_status = save_scanner_status;
