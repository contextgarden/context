...
/*tex
    ...
    The |\showtokens| command displays a token list. The |\showifs| command
    displays all currently active conditionals.

    The |\unexpanded| primitive prevents expansion of tokens much as the result
    from |\the| applied to a token variable. The |\detokenize| primitive converts
    a token list into a list of character tokens much as if the token list were
    written to a file. We use the fact that the command modifiers for
    |\unexpanded| and |\detokenize| are odd whereas those for |\the| and
    |\showthe| are even.

    The |protected| feature of \ETEX\ defines the |\protected| prefix command for
    macro definitions. Such macros are protected against expansions when lists of
    expanded tokens are built, e.g., for |\edef| or during |\write|.
    ...
    The |\interlinepenalties|, |\clubpenalties|, |\widowpenalties|, and
    |\displaywidowpenalties| commands allow to define arrays of penalty values to
    be used instead of the corresponding single values.

    Watch out, we have |\eTeXVersion| and |\eTeXversion|.

*/

/*tex

    The symbolic names for glue parameters are put into \TEX's hash table by
    using the routine called |primitive|, defined below. Let us enter them now,
    so that we don't have to list all those parameter names anywhere else.

    Many of \TEX's primitives need no |equiv|, since they are identifiable by
    their |eq_type| alone. These primitives are loaded into the hash table.

    The processing of |\input| involves the |start_input| subroutine, which
    will be declared later; the processing of |\endinput| is trivial.

    The hash table is initialized with |\count|, |\attribute|, |\dimen|, |\skip|,
    and |\muskip| all having |register| as their command code; they are
    distinguished by the |chr_code|, which is either |int_val|, |attr_val|,
    |dimen_val|, |glue_val|, or |mu_val|.
...
void initialize_commands(void)
{
    ...
    primitive_luatex("firstvalidlanguage",        assign_int_cmd,            int_base + first_valid_language_code,        int_base);
    primitive_luatex("fixupboxesmode",            assign_int_cmd,            int_base + fixup_boxes_code,                 int_base);
    primitive_luatex("glyphdatafield",            assign_int_cmd,            int_base + character_data_code,              int_base);
    ...
    primitive_luatex("boundary",                  boundary_cmd,              1,                                           0);
    primitive_luatex("noboundary",                boundary_cmd,              0,                                           0);
    primitive_luatex("protrusionboundary",        boundary_cmd,              2,                                           0);
    primitive_luatex("wordboundary",              boundary_cmd,              3,                                           0);
 ...
 /* primitive_tex("endgroup",                     end_group_cmd,             0,                                           0); */
    ...
    primitive_etex("futureexpand",                expand_after_cmd,          2,                                           0);
    primitive_etex("futureexpandis",              expand_after_cmd,          3,                                           0);

    primitive_tex("halign",                       halign_cmd,                0,                                           0);

    primitive_tex("hrule",                        hrule_cmd,                 0,                                           0);
    primitive_luatex("nohrule",                   hrule_cmd,                 1,                                           0);
    ...
    primitive_luatex("noexpand",                  no_expand_cmd,             0,                                           0);
 ...
 /* primitive_tex("nullfont",                     set_font_cmd,              null_font,                                   0); */
    ...
    primitive_luatex("novrule",                   vrule_cmd,                 1,                                           0);
    ...
    primitive_luatex("ifabsnum",                  if_test_cmd,               if_abs_int_code,                             0);
    primitive_luatex("ifabsdim",                  if_test_cmd,               if_abs_dim_code,                             0);
    primitive_luatex("ifchkdim",                  if_test_cmd,               if_chk_dim_code,                             0);
    primitive_luatex("ifchknum",                  if_test_cmd,               if_chk_int_code,                             0);
    primitive_luatex("ifcmpdim",                  if_test_cmd,               if_cmp_dim_code,                             0);
    primitive_luatex("ifcmpnum",                  if_test_cmd,               if_cmp_int_code,                             0);
    primitive_luatex("ifdimval",                  if_test_cmd,               if_val_dim_code,                             0);
    primitive_luatex("ifnumval",                  if_test_cmd,               if_val_int_code,                             0);
    primitive_luatex("iftok",                     if_test_cmd,               if_tok_code,                                 0);
 ...
 /* primitive_tex("fi",                           fi_or_else_cmd,            fi_code,                                     0); */
    ...
    primitive_luatex("boxorientation",            assign_box_direction_cmd,  1,                                           0);
    primitive_luatex("boxxoffset",                assign_box_direction_cmd,  2,                                           0);
    primitive_luatex("boxyoffset",                assign_box_direction_cmd,  3,                                           0);
    primitive_luatex("boxxmove",                  assign_box_direction_cmd,  4,                                           0);
    primitive_luatex("boxymove",                  assign_box_direction_cmd,  5,                                           0);
    ...
    primitive_tex("shipout",                      leader_ship_cmd,           a_leaders - 2,                               0);
 ...
 /* primitive_tex("middle",                       left_right_cmd,            middle_noad_side,                            0); */
    ...
    primitive_luatex("Uleft",                     left_right_cmd,            10 + left_noad_side,                         0);
    primitive_luatex("Umiddle",                   left_right_cmd,            10 + middle_noad_side,                       0);
    primitive_luatex("Uright",                    left_right_cmd,            10 + right_noad_side,                        0);
    ...
    primitive_luatex("glet",                      let_cmd,                   0,                                           0);
    ...
    primitive_luatex("endlocalcontrol",           end_local_cmd,             0,                                           0);

    primitive_luatex("savecatcodetable",          catcode_table_cmd,         save_cat_code_table_code,                    0);
    primitive_luatex("initcatcodetable",          catcode_table_cmd,         init_cat_code_table_code,                    0);

    primitive_luatex("matholdmode",               assign_int_cmd,            int_base + math_old_mode_code,               int_base);
 ...
 /* primitive_tex("cr",                           car_ret_cmd,               cr_code,                                     cr_code); */
    ...
    /*tex
        ...
        A bunch of commands that need a special treatment, so we delayed their
        initialization. They can be find above commented.
