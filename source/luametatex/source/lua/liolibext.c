...
/*tex
    ...
    Lua doesn't have cardinals so basically we could stick to integers
    and accept that we have a limited range.
...
/*tex
    ...
    A few helpers to avoid reading numbers as strings. For now we put them in
    their own namespace. We also have a few helpers that can make \IO\ functions
    \TEX\ friendly.
...
static int writecardinal1(lua_State *L)
{
    FILE *f = tofile(L);
    int n = lua_tointeger(L,2);
    putc(n & 0xFF,f);
...
static int writecardinal2(lua_State *L)
{
    FILE *f = tofile(L);
    int n = lua_tointeger(L,2);
    putc((n >> 8) & 0xFF,f);
...
static int writecardinal2_le(lua_State *L)
{
    FILE *f = tofile(L);
    int n = lua_tointeger(L,2);
    putc( n       & 0xFF,f);
...
static int writecardinal3(lua_State *L)
{
    FILE *f = tofile(L);
    int n = lua_tointeger(L,2);
    putc((n >> 16) & 0xFF,f);
...
static int writecardinal3_le(lua_State *L)
{
    FILE *f = tofile(L);
    int n = lua_tointeger(L,2);
    putc( n        & 0xFF,f);
...
static int writecardinal4(lua_State *L)
{
    FILE *f = tofile(L);
    int n = lua_tointeger(L,2);
    putc((n >> 24) & 0xFF,f);
...
static int writecardinal4_le(lua_State *L)
{
    FILE *f = tofile(L);
    int n = lua_tointeger(L,2);
    putc( n        & 0xFF,f);
...
static const luaL_Reg fiolib[] = {
    ...
    { "writecardinal1",    writecardinal1 },
    { "writecardinal2",    writecardinal2 },
    { "writecardinal3",    writecardinal3 },
    { "writecardinal4",    writecardinal4 },

    { "writecardinal1le",  writecardinal1    },
    { "writecardinal2le",  writecardinal2_le },
    { "writecardinal3le",  writecardinal3_le },
    { "writecardinal4le",  writecardinal4_le },
...
static const luaL_Reg siolib[] = {
    ...
    { "readinteger1_le",   readinteger1      },
    { "readinteger2_le",   readinteger2_s_le },
    { "readinteger3_le",   readinteger3_s_le },
    { "readinteger4_le",   readinteger4_s_le },
...
int luaopen_fio(lua_State *L) {
    lua_newtable(L);
    luaL_setfuncs(L, fiolib, 0);
    return 1;
}

int luaopen_sio(lua_State *L) {
    lua_newtable(L);
    luaL_setfuncs(L, siolib, 0);
    return 1;
}
