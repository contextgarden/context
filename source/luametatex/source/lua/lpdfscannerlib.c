...
/*tex So not |luatex-common.h| as that gives conflicts: */
...
static void *priv_malloc(size_t size)
{
...
static void *priv_realloc(void *old_ptr, size_t size)
{
...
# define reallocarray(ptr,type,size) ((type*)priv_realloc(ptr,(size+1)*sizeof(type)))

# define define_buffer(a) \
  char *a = (char *)priv_malloc (INITBUFSIZE); \
...
# define check_overflow(a, wsize) do { \
    if (wsize >= a##_size) { \
        ...
        a = (char *) reallocarray(a, char, (unsigned) nsize); \
...
{
    Token *token = (Token *) priv_malloc(sizeof(Token));
...
static int scanner_scan(lua_State * L)
{
    ...
    self->_operandstack = (Token **) priv_malloc(MAXOPERANDS * sizeof(Token));
        ...
        while (1) {
            ...
            if (lua_type(L, -1) == LUA_TUSERDATA) {
                ...
                if (ud != NULL) {
                    ...
                    uin = (udstruct *) ud;
                    rover = self->_streams;
                    item = (ObjectList *) priv_malloc(sizeof(ObjectList));
        ...
        if (ud != NULL) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (obj->type == PPSTREAM) {
                    ObjectList *strm = self->_streams;
                    ObjectList *item = (ObjectList *) priv_malloc(sizeof(ObjectList));
...
static void operandstack_backup(scannerdata * self)
{
    ...
    for (; i >= 0; i--) {
        if ((int) self->_operandstack[i]->type == backupstop) {
            balance++;
        } else if ((int) self->_operandstack[i]->type == backupstart) {
            balance--;
        }
        if (balance == 0) {
            break;
...
static int scanner_popsingular(lua_State * L, int token_type)
{
    ...
    if (token == NULL || ((int) token->type != token_type)) {
        return 0;
    }
    /*tex
...
LUALIB_API int luaopen_pdfscanner(lua_State * L)
{
    ...
    luaL_setfuncs(L, scannerlib_meta, 0);
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_setfuncs(L, scannerlib_m, 0);
    lua_newtable(L);
    luaL_setfuncs(L, scannerlib, 0);
    return 1;
}
