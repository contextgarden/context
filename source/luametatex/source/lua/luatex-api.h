...
/*tex
    ...
    In this file we collect all kind of interface stuff related to \LUA. It is a
    large file because we also create \LUA\ string entries which speeds up the
    interfacing.
...
/*tex
    ...
    In the beginning we had multiple \LUA\ states but that didn't work out well
    if you want to intercace to the \TEX\ kernel. So, we dropped that and now
    have only one main state.
...
extern lua_state_info lua_state;

# define Luas lua_state.lua_instance
...
extern int luaopen_flate(lua_State * L);
...
/*tex We finetune the string hasher. */
...
/*tex
    ...
    Now comes a large section. Here we create and use macros that preset the
    access pointers (indices) to keys which is faster in comparison. We also
    handle the arrays that hold the information about what fields there are
    in nodes.
...
# define lua_roundnumber(a,b)  (int)((double)lua_tonumber(a,b)+0.5)
# define lua_uroundnumber(a,b) (unsigned int)((double)(lua_tonumber(a,b)+0.5))

/*tex

    Currently we sometimes use numbers and sometimes strings in node properties.
    We can make that consistent by having a check on number and if not then
    assign a string. The strings are prehashed and we make a bunch of lua tables
    that have these values. We can preassign these at startup time.
...
make_lua_key(ScriptPercentScaleDown); \
make_lua_key(ScriptScriptPercentScaleDown); \
make_lua_key(DelimitedSubFormulaMinHeight); \
make_lua_key(DisplayOperatorMinHeight); \
make_lua_key(MathLeading); \
make_lua_key(AxisHeight); \
make_lua_key(AccentBaseHeight); \
make_lua_key(FlattenedAccentBaseHeight); \
make_lua_key(SubscriptShiftDown); \
make_lua_key(SubscriptTopMax); \
make_lua_key(SubscriptBaselineDropMin); \
make_lua_key(SuperscriptShiftUp); \
make_lua_key(SuperscriptShiftUpCramped); \
make_lua_key(SuperscriptBottomMin); \
make_lua_key(SuperscriptBaselineDropMax); \
make_lua_key(SubSuperscriptGapMin); \
make_lua_key(SuperscriptBottomMaxWithSubscript); \
make_lua_key(SpaceAfterScript); \
make_lua_key(UpperLimitGapMin); \
make_lua_key(UpperLimitBaselineRiseMin); \
make_lua_key(LowerLimitGapMin); \
make_lua_key(LowerLimitBaselineDropMin); \
make_lua_key(StackTopShiftUp); \
make_lua_key(StackTopDisplayStyleShiftUp); \
make_lua_key(StackBottomShiftDown); \
make_lua_key(StackBottomDisplayStyleShiftDown); \
make_lua_key(StackGapMin); \
make_lua_key(StackDisplayStyleGapMin); \
make_lua_key(StretchStackTopShiftUp); \
make_lua_key(StretchStackBottomShiftDown); \
make_lua_key(StretchStackGapAboveMin); \
make_lua_key(StretchStackGapBelowMin); \
make_lua_key(FractionNumeratorShiftUp); \
make_lua_key(FractionNumeratorDisplayStyleShiftUp); \
make_lua_key(FractionDenominatorShiftDown); \
make_lua_key(FractionDenominatorDisplayStyleShiftDown); \
make_lua_key(FractionNumeratorGapMin); \
make_lua_key(FractionNumeratorDisplayStyleGapMin); \
make_lua_key(FractionRuleThickness); \
make_lua_key(FractionDenominatorGapMin); \
make_lua_key(FractionDenominatorDisplayStyleGapMin); \
make_lua_key(SkewedFractionHorizontalGap); \
make_lua_key(SkewedFractionVerticalGap); \
make_lua_key(OverbarVerticalGap); \
make_lua_key(OverbarRuleThickness); \
make_lua_key(OverbarExtraAscender); \
make_lua_key(UnderbarVerticalGap); \
make_lua_key(UnderbarRuleThickness); \
make_lua_key(UnderbarExtraDescender); \
make_lua_key(RadicalVerticalGap); \
make_lua_key(RadicalDisplayStyleVerticalGap); \
make_lua_key(RadicalRuleThickness); \
make_lua_key(RadicalExtraAscender); \
make_lua_key(RadicalKernBeforeDegree); \
make_lua_key(RadicalKernAfterDegree); \
make_lua_key(RadicalDegreeBottomRaisePercent); \
make_lua_key(MinConnectorOverlap); \
make_lua_key(SubscriptShiftDownWithSuperscript); \
make_lua_key(FractionDelimiterSize); \
make_lua_key(FractionDelimiterDisplayStyleSize); \
make_lua_key(NoLimitSubFactor); \
make_lua_key(NoLimitSupFactor);
...
init_lua_key(ScriptPercentScaleDown); \
init_lua_key(ScriptScriptPercentScaleDown); \
init_lua_key(DelimitedSubFormulaMinHeight); \
init_lua_key(DisplayOperatorMinHeight); \
init_lua_key(MathLeading); \
init_lua_key(AxisHeight); \
init_lua_key(AccentBaseHeight); \
init_lua_key(FlattenedAccentBaseHeight); \
init_lua_key(SubscriptShiftDown); \
init_lua_key(SubscriptTopMax); \
init_lua_key(SubscriptBaselineDropMin); \
init_lua_key(SuperscriptShiftUp); \
init_lua_key(SuperscriptShiftUpCramped); \
init_lua_key(SuperscriptBottomMin); \
init_lua_key(SuperscriptBaselineDropMax); \
init_lua_key(SubSuperscriptGapMin); \
init_lua_key(SuperscriptBottomMaxWithSubscript); \
init_lua_key(SpaceAfterScript); \
init_lua_key(UpperLimitGapMin); \
init_lua_key(UpperLimitBaselineRiseMin); \
init_lua_key(LowerLimitGapMin); \
init_lua_key(LowerLimitBaselineDropMin); \
init_lua_key(StackTopShiftUp); \
init_lua_key(StackTopDisplayStyleShiftUp); \
init_lua_key(StackBottomShiftDown); \
init_lua_key(StackBottomDisplayStyleShiftDown); \
init_lua_key(StackGapMin); \
init_lua_key(StackDisplayStyleGapMin); \
init_lua_key(StretchStackTopShiftUp); \
init_lua_key(StretchStackBottomShiftDown); \
init_lua_key(StretchStackGapAboveMin); \
init_lua_key(StretchStackGapBelowMin); \
init_lua_key(FractionNumeratorShiftUp); \
init_lua_key(FractionNumeratorDisplayStyleShiftUp); \
init_lua_key(FractionDenominatorShiftDown); \
init_lua_key(FractionDenominatorDisplayStyleShiftDown); \
init_lua_key(FractionNumeratorGapMin); \
init_lua_key(FractionNumeratorDisplayStyleGapMin); \
init_lua_key(FractionRuleThickness); \
init_lua_key(FractionDenominatorGapMin); \
init_lua_key(FractionDenominatorDisplayStyleGapMin); \
init_lua_key(SkewedFractionHorizontalGap); \
init_lua_key(SkewedFractionVerticalGap); \
init_lua_key(OverbarVerticalGap); \
init_lua_key(OverbarRuleThickness); \
init_lua_key(OverbarExtraAscender); \
init_lua_key(UnderbarVerticalGap); \
init_lua_key(UnderbarRuleThickness); \
init_lua_key(UnderbarExtraDescender); \
init_lua_key(RadicalVerticalGap); \
init_lua_key(RadicalDisplayStyleVerticalGap); \
init_lua_key(RadicalRuleThickness); \
init_lua_key(RadicalExtraAscender); \
init_lua_key(RadicalKernBeforeDegree); \
init_lua_key(RadicalKernAfterDegree); \
init_lua_key(RadicalDegreeBottomRaisePercent); \
init_lua_key(MinConnectorOverlap); \
init_lua_key(SubscriptShiftDownWithSuperscript); \
init_lua_key(FractionDelimiterSize); \
init_lua_key(FractionDelimiterDisplayStyleSize); \
init_lua_key(NoLimitSubFactor); \
init_lua_key(NoLimitSupFactor); \
...
/*tex These keys have to available to different files. */
...
use_lua_key(yoffset);
use_lua_key(ScriptPercentScaleDown);
use_lua_key(ScriptScriptPercentScaleDown);
use_lua_key(DelimitedSubFormulaMinHeight);
use_lua_key(DisplayOperatorMinHeight);
use_lua_key(MathLeading);
use_lua_key(AxisHeight);
use_lua_key(AccentBaseHeight);
use_lua_key(FlattenedAccentBaseHeight);
use_lua_key(SubscriptShiftDown);
use_lua_key(SubscriptTopMax);
use_lua_key(SubscriptBaselineDropMin);
use_lua_key(SuperscriptShiftUp);
use_lua_key(SuperscriptShiftUpCramped);
use_lua_key(SuperscriptBottomMin);
use_lua_key(SuperscriptBaselineDropMax);
use_lua_key(SubSuperscriptGapMin);
use_lua_key(SuperscriptBottomMaxWithSubscript);
use_lua_key(SpaceAfterScript);
use_lua_key(UpperLimitGapMin);
use_lua_key(UpperLimitBaselineRiseMin);
use_lua_key(LowerLimitGapMin);
use_lua_key(LowerLimitBaselineDropMin);
use_lua_key(StackTopShiftUp);
use_lua_key(StackTopDisplayStyleShiftUp);
use_lua_key(StackBottomShiftDown);
use_lua_key(StackBottomDisplayStyleShiftDown);
use_lua_key(StackGapMin);
use_lua_key(StackDisplayStyleGapMin);
use_lua_key(StretchStackTopShiftUp);
use_lua_key(StretchStackBottomShiftDown);
use_lua_key(StretchStackGapAboveMin);
use_lua_key(StretchStackGapBelowMin);
use_lua_key(FractionNumeratorShiftUp);
use_lua_key(FractionNumeratorDisplayStyleShiftUp);
use_lua_key(FractionDenominatorShiftDown);
use_lua_key(FractionDenominatorDisplayStyleShiftDown);
use_lua_key(FractionNumeratorGapMin);
use_lua_key(FractionNumeratorDisplayStyleGapMin);
use_lua_key(FractionRuleThickness);
use_lua_key(FractionDenominatorGapMin);
use_lua_key(FractionDenominatorDisplayStyleGapMin);
use_lua_key(SkewedFractionHorizontalGap);
use_lua_key(SkewedFractionVerticalGap);
use_lua_key(OverbarVerticalGap);
use_lua_key(OverbarRuleThickness);
use_lua_key(OverbarExtraAscender);
use_lua_key(UnderbarVerticalGap);
use_lua_key(UnderbarRuleThickness);
use_lua_key(UnderbarExtraDescender);
use_lua_key(RadicalVerticalGap);
use_lua_key(RadicalDisplayStyleVerticalGap);
use_lua_key(RadicalRuleThickness);
use_lua_key(RadicalExtraAscender);
use_lua_key(RadicalKernBeforeDegree);
use_lua_key(RadicalKernAfterDegree);
use_lua_key(RadicalDegreeBottomRaisePercent);
use_lua_key(MinConnectorOverlap);
use_lua_key(SubscriptShiftDownWithSuperscript);
use_lua_key(FractionDelimiterSize);
use_lua_key(FractionDelimiterDisplayStyleSize);
use_lua_key(NoLimitSubFactor);
use_lua_key(NoLimitSupFactor);
