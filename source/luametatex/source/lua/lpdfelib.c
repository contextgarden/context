...
static int pdfelib_getfromarray(lua_State * L)
{
    ...
    if (a != NULL) {
        unsigned int index = luaL_checkinteger(L, 2) - 1;
...
static int pdfelib_open(lua_State * L)
{
    ...
    if (d == NULL) {
        formatted_warning("pdfe lib","no valid pdf file '%s'\n",filename);
    } else {
...
static int pdfelib_new(lua_State * L)
{
    ...
    /* size of the stream */
    streamsize = (unsigned long long) luaL_checkinteger(L, 2);
    memstream = malloc((unsigned) (streamsize + 1));
    if (! memstream) {
...
static int pdfelib_getpage(lua_State * L)
{
    ...
    if (p == NULL) {
        ...
        return pushpage(L, p->document, luaL_checkinteger(L, 2));
    }
}
