/* lmplib.c
    ...
    along with LuaTeX; if not, see <http://www.gnu.org/licenses/>.

*/

/*tex

    This is a copy of the file in the mplib distribution but we put it here so that
    we can experiment without harming the original. Once found useful and ok we can
    backport to the main lib.

    Somehow we cannot comment |mplib_fig_png| and |mplib_fig_svg| so that has to be
    figured out some day. Their usage forces some inclusions.

    Todo: feed back some small consistency fixes to regular luatex lmplib.

*/

//# include <w2c/config.h>
# include "mpc/w2c/config.h"
# include <stdlib.h>
# include <string.h>
# ifdef HAVE_UNISTD_H
# include <unistd.h>
...
# include "mpc/mplib.h"
# include "mpc/mplibps.h"
# include "mpc/mplibsvg.h"
# include "mpc/mplibpng.h"
...
/*tex
    ...
    We pre-hash the \LUA\ strings which is much faster. The approach is similar
    to the one used at the \TEX\ end.
...
static const char *interaction_options[] = {
    "unknown", "batch", "nonstop", "scroll", "errorstop",
    NULL
};
...
static const char *mplib_filetype_names[] = {
    "term", "error", /* internal */
    "mp", "log",
    "mem", /* actually there is no mem file */
    "ps", "png",  /* and we also hav eno output like this */
    "tfm", "map", "pfb", "enc",
    NULL
};
...
/*tex
    ...
    We start by defining the needed callback routines for the library. We could
    store them per instance but it has no advantages so that will be done when we
    feel the need.

*/

static int file_finder_id  = 0;
static int script_error_id = 0;
static int run_script_id   = 0;
static int make_text_id    = 0;
...
static void mplib_script_warning(const char *one, const char *two)
{
    fprintf(stdout,"mplib warning: %s: %s\n",one,two);
}

static int mplib_register_function(lua_State * L, int old_id)
{
...
{
    file_finder_id = mplib_register_function(L, file_finder_id);
    return (! file_finder_id);
}

static int mplib_script_error_function(lua_State * L)
{
    script_error_id = mplib_register_function(L, script_error_id);
    return (! script_error_id);
}

static int mplib_run_script_function(lua_State * L)
{
    run_script_id = mplib_register_function(L, run_script_id);
    return (! run_script_id);
}

static int mplib_make_text_function(lua_State * L)
{
    make_text_id = mplib_register_function(L, make_text_id);
    return (! make_text_id);
}
...
static char *mplib_find_file(MP mp, const char *fname, const char *fmode, int ftype)
{
    if (file_finder_id) {
        ...
        lua_checkstack(L, 4);
        lua_rawgeti(L, LUA_REGISTRYINDEX, file_finder_id);
        ...
        if (lua_pcall(L, 3, 1, 0) != 0) {
            mplib_script_warning("find file",lua_tostring(L, -1)); /* doesn't use lua */
            return NULL;
        } else {
            char *s = NULL;
...
static void mplib_script_error(MP mp, const char *str)
{
    if (script_error_id) {
        lua_State *L = (lua_State *)mp_userdata(mp);
        lua_checkstack(L, 1);
        lua_rawgeti(L, LUA_REGISTRYINDEX, script_error_id);
...
static char *mplib_run_script(MP mp, const char *str)
{
    if (run_script_id) {
        lua_State *L = (lua_State *) mp_userdata(mp);
        lua_checkstack(L, 1);
        lua_rawgeti(L, LUA_REGISTRYINDEX, run_script_id);
        lua_pushstring(L, str);
        if (lua_pcall(L, 1, 1, 0) != 0) {
            mplib_script_warning("run script",lua_tostring(L, -1)); /* doesn't use lua */
        } else {
            const char *x = lua_tostring(L, -1);
            char *s = NULL;
            if (x != NULL) {
...
static char *mplib_make_text(MP mp, const char *str, int mode)
{
    if (make_text_id) {
        lua_State *L = (lua_State *)mp_userdata(mp);
        lua_checkstack(L, 2);
        lua_rawgeti(L, LUA_REGISTRYINDEX, make_text_id);
        lua_pushstring(L, str);
...
{
    MP *mp_ptr = lua_newuserdata(L, sizeof(MP *));
    if (mp_ptr != NULL) {
                ...
                switch (mplib_parms[i].idx) {
                    case P_ERROR_LINE:
                        ...
                        if (options->error_line > 250) {
                            /*tex Maybe we should permit a larger value: */
...
static int mplib_execute(lua_State * L)
{
    ...
    if (*mp_ptr != NULL && lua_isstring(L, 2)) {
        size_t l;
        char *s = strdup(lua_tolstring(L, 2, &l));
...
static int mplib_char_dimension(lua_State * L, int t)
{
    MP *mp_ptr = is_mp(L, 1);
    if (*mp_ptr != NULL) {
        char *fname = strdup(luaL_checkstring(L,2));
    ...
    do {
        ...
        switch (mp_knot_right_type(mp,p)) {
            case mp_endpoint:
                ...
                break;
            case mp_explicit:
                printf ("..controls (%g,%g)", mp_number_as_double(mp,mp_knot_right_x(mp,p)),
...
static int mplib_solve_path(lua_State * L)
{
    ...
    mp_ptr = is_mp(L, 1);
    if (*mp_ptr == NULL || (!lua_istable(L,2)) || lua_rawlen(L,2) <= 0 || (!lua_isboolean(L,3))) {
...
static int mplib_fig_postscript(lua_State * L)
{
    ...
    if (mp_ps_ship_out(*hh, prologues, procset) && (res = mp_rundata((*hh)->parent)) && (res->ship_out.size != 0)) {
...
static int mplib_fig_svg(lua_State * L)
{
    ...
    struct mp_edge_object **hh = is_fig(L, 1);
    int prologues = (int)luaL_optnumber(L, 2, (lua_Number)-1);
    if (mp_svg_ship_out(*hh, prologues) && (res = mp_rundata((*hh)->parent))  && (res->ship_out.size != 0)) {
        lua_pushstring(L, res->ship_out.data);
    } else {
        lua_pushnil(L);
    }
...
static int mplib_fig_png(lua_State * L)
{
    ...
    if (mp_png_ship_out(*hh, string) && (res = mp_rundata((*hh)->parent)) && (res->ship_out.size != 0)) {
...
static int mplib_gr_peninfo(lua_State * L) {
    ...
    */
    mplib_push_S(width); lua_pushnumber(L,width); lua_rawset(L, -3);
    mplib_push_S(rx);    lua_pushnumber(L,rx);    lua_rawset(L, -3);
    mplib_push_S(sx);    lua_pushnumber(L,sx);    lua_rawset(L, -3);
    mplib_push_S(sy);    lua_pushnumber(L,sy);    lua_rawset(L, -3);
    mplib_push_S(ry);    lua_pushnumber(L,ry);    lua_rawset(L, -3);
    mplib_push_S(tx);    lua_pushnumber(L,tx);    lua_rawset(L, -3);
    mplib_push_S(ty);    lua_pushnumber(L,ty);    lua_rawset(L, -3);
    return 1;
}
    ...
    if (*mp != NULL) {
        ...
        if (s != NULL) {
            ...
            if (p != NULL) {
                int i = 1;
                mp_knot h = p;
                lua_newtable(L); /* todo: precalculate size */
                do {
...
static void mplib_push_color(lua_State * L, struct mp_graphic_object *p)
{
    ...
    if (p != NULL) {
        ...
        lua_createtable(L,object_color_model == mp_cmyk_model ? 4 : object_color_model == mp_rgb_model ? 3 : 1,0);
...
int luaopen_mplib(lua_State * L)
{
    ...
    luaL_setfuncs(L, mplib_gr_meta, 0);
    luaL_newmetatable(L, MPLIB_FIG_METATABLE);
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_setfuncs(L, mplib_fig_meta, 0);
    luaL_newmetatable(L, MPLIB_METATABLE);
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_setfuncs(L, mplib_meta, 0);
    luaL_setfuncs(L, mplib_d, 0);
    lua_newtable(L);
    luaL_setfuncs(L, mplib_m, 0);
    return 1;
}
