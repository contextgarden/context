...
struct tex_language *new_language(int n)
{
    ...
    if (l < (MAX_TEX_LANGUAGES - 1) && tex_languages[l] == NULL) {
        lang = malloc(sizeof(struct tex_language));
        ...
        if (saving_hyph_codes_par) {
            /*tex For now, we might just use specific value for whatever task. */
...
{
    struct tex_language *l = get_language((int) n);
    return (l == NULL) ? -1 : (int) l->pre_hyphen_char;
}

int get_post_hyphen_char(int n)
{
    struct tex_language *l = get_language((int) n);
    return (l == NULL) ? -1 : (int) l->post_hyphen_char;
}

int get_pre_exhyphen_char(int n)
{
    struct tex_language *l = get_language((int) n);
    return (l == NULL) ? -1 : (int) l->pre_exhyphen_char;
}

int get_post_exhyphen_char(int n)
{
    struct tex_language *l = get_language((int) n);
    return (l == NULL) ? -1 : (int) l->post_exhyphen_char;
}
...
{
    struct tex_language *l = get_language((int) n);
    return (l == NULL) ? -1 : (int) l->hyphenation_min;
}
...
{
    if (lang != NULL && lang->patterns != NULL) {
        ...
        if (u == '-') {
            ...
            if (items != 3) {
                *cleaned = NULL;
                ...
                return s;
            } else if (uword[i] == '[' && uword[i+1] >= '0' && uword[i+1] <= '9' && uword[i+2] == ']') {
                i += 3;
            }
        } else {
            STORE_CHAR(id,u);
...
{
    if (lang != NULL && lang->exceptions != 0) {
...
{
    halfword g;
    halfword n = new_node(disc_node, syllable_disc);
...
{
    halfword p = new_node(glyph_node, 0); /* use new_* */
    set_to_character(p);
    character(p) = c;
    if (t) {
        couple_nodes(t, p);
...
static char *hyphenation_exception(int exceptions, char *w)
{
    ...
    if (lua_istable(Luas, -1)) {
        ...
        if (lua_type(Luas, -1) == LUA_TSTRING) {
            ret = strdup(lua_tostring(Luas, -1));
        }
        lua_pop(Luas, 2);
    } else {
        lua_pop(Luas, 1);
    ...
    if (lang != NULL && lang->exceptions != 0) {
        ...
        if (lua_istable(Luas, -1)) {
            ...
            while (lua_next(Luas, -2) != 0) {
                ...
                if (current + 2 + l > size) {
                    ret = realloc(ret, (unsigned) ((size + size / 5) + current + l + 1024));
...
static halfword find_exception_part(unsigned int *j, unsigned int *uword, int len)
{
    ...
    while (i < (unsigned) len && uword[i + 1] != '}') {
        if (g == null) {
            gg = new_char(0, (int) uword[i + 1], 0); /* todo: data */
            g = gg;
        } else {
            halfword s = new_char(0, (int) uword[i + 1], 0); /* todo: data */
            couple_nodes(g, s);
            g = vlink(g);
        }
        i++;
                ...
                while (q != null) {
                    ...
                    if (type(q) == disc_node) {
                        ...
                        if (q == replace) {
                            ...
                            try_couple_nodes(alink(q),nb);
                        }
                        /*tex Append the glyph (one). */
                        try_couple_nodes(nb,n);
                        /*tex Flush the disc. */
                        flush_node(q);
                    }
                    q = n ;
...
/*tex
    ...
    The following description is no longer valid for \LUATEX. Although we use
    the same algorithm for hyphenation, it is not longer integrated in the
    par builder. Instead it is a separate run over the node list, preceding
    the line-breaking routine, possibly replaced by a callback. We keep the
    description here because the principles remain.
    ...
    belongs to a \quotation {potentially hyphenatable part} of the current
    paragraph. This is a sequence of nodes $p_0p_1\ldots p_m$ where $p_0$ is a
    glue node, $p_1\ldots p_{m-1}$ are either character or ligature or whatsit or
    implicit kern nodes, and $p_m$ is a glue or penalty or insertion or adjust or
    mark or whatsit or explicit kern node. (Therefore hyphenation is disabled by
    boxes, math formulas, and discretionary nodes already inserted by the user.)
    The ligature nodes among $p_1\ldots p_{m-1}$ are effectively expanded into
    the original non-ligature characters; the kern nodes and whatsits are
    ignored. Each character |c| is now classified as either a nonletter (if
...
static int valid_wordend(halfword s, halfword strict_bound)
{
    if (s) {
        ...
        return (r == null || (type(r) == glyph_node && is_simple_character(r) && clang != char_lang(r))
                          ||  type(r) == glue_node
                          ||  type(r) == penalty_node
...
void hnj_hyphenation(halfword head, halfword tail)
{
    ...
    if (tail) {
        ...
        /*tex
            ...
            \stopitemize
        */
        while (r && (type(r) != glyph_node || !is_simple_character(r))) {
            r = vlink(r);
        }
        /*tex
        ...
        */
        ...
        if (r) {
            ...
            int wordlen = 0;
            char *hy = utf8word;
            halfword wordstart = null;
            halfword left = null;
            halfword right = null;
            halfword expstart = null;
            halfword save_tail1 = vlink(tail);
            halfword s = new_penalty(0,word_penalty);
            int compound_hyphen = compound_hyphen_mode_par;
            ...
            while (r) {
                ...
                if (explicit_hyphen) {
                    ...
                    if ((automatic_hyphen_mode_par == 0 || automatic_hyphen_mode_par == 1) && (t != null) && ((type(t) == glyph_node) && (character(t) != ex_hyphen_char_par))) {
                        ...
                        /*tex we jump over the sequence of hyphens */
                       while (t && (type(t) == glyph_node) && (character(t) == ex_hyphen_char_par)) {
                            r = t ;
                            t = vlink(r) ;
                        }
                        if (!t) {
...
{
    if (head && vlink(head)) {
        int callback_id = callback_defined(hyphenate_callback);
        if (callback_id > 0) {
            int top;
...
/*tex
    ...
    The |\prehyphenchar| command sets the |pre_break| character, and
    |\posthyphenchar| the |post_break| character. Their respective defaults are
    |ASCII\ hyphen (\type {-}) and zero (\type {nul}).
...
/*tex
    ...
    The |\preexhyphenchar| command sets the |pre_break| character, and
    |\postexhyphenchar| command the |post_break| character. Their defaults are
    both zero (\type {nul}).
