...
/*
    ...
    This file is derived from libhnj which is is dual licensed under LGPL and
    MPL. Boilerplate for both licenses follows.
    ...
    1.0 (the "MPL"); you may not use this file except in compliance with the MPL.
    You may obtain a copy of the MPL at http://www.mozilla.org/MPL/
...
{
    HashEntry *e;
    int i = (int) (hnj_string_hash(key) % HASH_SIZE);
...
static int hnj_get_state(HyphenDict * dict, const unsigned char *str, int *state_num)
{
    *state_num = state_lookup(dict->state_num, str);
    ...
    if (!(dict->num_states & (dict->num_states - 1))) {
        dict->states = hnj_realloc(dict->states, (int) ((dict->num_states << 1) * (int) sizeof(HyphenState)));
    }
...
static void hnj_add_trans(HyphenDict * dict, int state1, int state2, int uni_ch)
{
    ...
    /*tex
        ...
        This test was a bit too strict, it is quite normal for old patterns to
        have chars in the range 0-31 or 127-159 (inclusive). To ease the
...
unsigned char *hnj_serialize(HyphenDict * dict)
{
    ...
    HashIter *v = new_HashIter(dict->patterns);
    ...
    while (nextHash(v, &word)) {
        ...
        for (l1 = 1; l1 <= wordsize; l1++) {
            ...
            for (j1 = 1; j1 <= l1; j1++) {
                ...
                if ((subpat_pat = hyppat_lookup(dict->patterns, word + i1, j1)) != NULL) {
                    char *newpat_pat;
                    if ((newpat_pat = hyppat_lookup(dict->merged, word, l1)) == NULL) {
                        char *neworg;
                        unsigned char *newword = (unsigned char *) malloc((size_t) (l1 + 1));
                        int e1 = 0;
    ...
    int length, halfword left, halfword right, lang_variables * lan)
{
    ...
    /*tex
        Pattern is |.word.| and |word_len| is 4, |ext_word_len| is 6 and
        |hyphens| is 7; drop first two and stop after |word_len-1|.
     */
