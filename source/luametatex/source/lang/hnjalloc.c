...
/*
    ...
    LibHnj - a library for high quality hyphenation and justification Copyright
    (C) 1998 Raph Levien, (C) 2001 ALTLinux, Moscow
...
void *hnj_malloc(int size)
{
    ...
    if (p == NULL) {
        formatted_error("hyphenation", "allocating %d bytes failed\n", size);
    }
    return p;
}
...
void *hnj_realloc(void *p, int size)
{
    ...
    if (p == NULL) {
        formatted_error("hyphenation", "reallocating %d bytes failed\n", size);
    }
    return p;
}
