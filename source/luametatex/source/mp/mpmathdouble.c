...
static void mp_double_set_from_int(mp_number *A, int B)
{
    A->data.dval = B;
}

static void mp_double_set_from_boolean(mp_number *A, int B)
{
    A->data.dval = B;
}
...
static int mp_double_to_int(mp_number *A)
{
    return (int) (A->data.dval);
}

static int mp_double_to_boolean(mp_number *A)
{
    return (int) (A->data.dval);
}
...
/*tex 
    ...
    This is the same one as in |mpmath.c|:

*/

static int mp_double_unscaled(mp_number *x_orig)
{
...
static void mp_double_m_unif_rand(MP mp, mp_number *ret, mp_number *x_orig)
{
    ...
    mp_double_allocate_clone(mp, &x, mp_scaled_type, x_orig);
    mp_double_allocate_abs(mp, &abs_x, mp_scaled_type, &x);
    mp_double_allocate_number(mp, &u, mp_scaled_type);
    mp_double_aux_next_unif_random(mp, &u);
    y.data.dval = abs_x.data.dval * u.data.dval;
    mp_double_free_number(mp, &u);
    if (mp_double_equal(&y, &abs_x)) {
        mp_double_clone(ret, &((math_data *)mp->math)->md_zero_t);
    } else if (mp_double_greater(&x, &((math_data *)mp->math)->md_zero_t)) {
        mp_double_clone(ret, &y);
    } else {
        mp_double_negated_clone(ret, &y);
    }
    mp_double_free_number(mp, &abs_x);
    mp_double_free_number(mp, &x);
...
static void mp_double_m_norm_rand(MP mp, mp_number *ret)
{
    ...
    mp_double_allocate_number(mp, &xa, mp_scaled_type);
    mp_double_allocate_number(mp, &abs_x, mp_scaled_type);
    mp_double_allocate_number(mp, &u, mp_scaled_type);
    mp_double_allocate_number(mp, &r, mp_scaled_type);
    do {
        do {
            mp_number v;
            mp_double_allocate_number(mp, &v, mp_scaled_type);
            mp_double_aux_next_random(mp, &v);
            mp_double_subtract(&v, &((math_data *)mp->math)->md_fraction_half_t);
            mp_double_number_take_fraction(mp, &xa, &((math_data *)mp->math)->md_sqrt_8_e_k, &v);
            mp_double_free_number(mp, &v);
            mp_double_aux_next_random(mp, &u);
            mp_double_clone(&abs_x, &xa);
            mp_double_abs(&abs_x);
        } while (! mp_double_less(&abs_x, &u));
        mp_double_number_make_fraction(mp, &r, &xa, &u);
        mp_double_clone(&xa, &r);
        mp_double_m_log(mp, &la, &u);
        mp_double_set_from_subtraction(&la, &((math_data *)mp->math)->md_twelve_ln_2_k, &la);
    } while (mp_double_ab_vs_cd(&((math_data *)mp->math)->md_one_k, &la, &xa, &xa) < 0);
    mp_double_clone(ret, &xa);
    mp_double_free_number(mp, &r);
    mp_double_free_number(mp, &abs_x);
    mp_double_free_number(mp, &la);
    mp_double_free_number(mp, &xa);
    mp_double_free_number(mp, &u);
}
...
static void mp_double_free_math(MP mp)
{
    ...
    mp_double_free_number(mp, &(mp->math->md_negative_one_eighty_deg_t));
    mp_double_free_number(mp, &(mp->math->md_fraction_one_t));
    mp_double_free_number(mp, &(mp->math->md_zero_t));
    mp_double_free_number(mp, &(mp->math->md_half_unit_t));
    mp_double_free_number(mp, &(mp->math->md_three_quarter_unit_t));
    mp_double_free_number(mp, &(mp->math->md_unity_t));
    mp_double_free_number(mp, &(mp->math->md_two_t));
    mp_double_free_number(mp, &(mp->math->md_three_t));
    mp_double_free_number(mp, &(mp->math->md_one_third_inf_t));
    mp_double_free_number(mp, &(mp->math->md_inf_t));
    mp_double_free_number(mp, &(mp->math->md_negative_inf_t));
    mp_double_free_number(mp, &(mp->math->md_warning_limit_t));
    mp_double_free_number(mp, &(mp->math->md_one_k));
    mp_double_free_number(mp, &(mp->math->md_sqrt_8_e_k));
    mp_double_free_number(mp, &(mp->math->md_twelve_ln_2_k));
    mp_double_free_number(mp, &(mp->math->md_coef_bound_k));
    mp_double_free_number(mp, &(mp->math->md_coef_bound_minus_1));
    mp_double_free_number(mp, &(mp->math->md_fraction_threshold_t));
    mp_double_free_number(mp, &(mp->math->md_half_fraction_threshold_t));
    mp_double_free_number(mp, &(mp->math->md_scaled_threshold_t));
    mp_double_free_number(mp, &(mp->math->md_half_scaled_threshold_t));
    mp_double_free_number(mp, &(mp->math->md_near_zero_angle_t));
    mp_double_free_number(mp, &(mp->math->md_p_over_v_threshold_t));
    mp_double_free_number(mp, &(mp->math->md_equation_threshold_t));
...
math_data *mp_initialize_double_math(MP mp)
{
    ...
    /* alloc */
    math->md_allocate        = mp_double_allocate_number;
    ...
    mp_double_allocate_number(mp, &math->md_precision_max, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_precision_min, mp_scaled_type);
    /* here are the constants for |scaled| objects */
    mp_double_allocate_number(mp, &math->md_epsilon_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_inf_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_negative_inf_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_warning_limit_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_one_third_inf_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_unity_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_two_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_three_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_half_unit_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_three_quarter_unit_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_zero_t, mp_scaled_type);
    /* |fractions| */
    mp_double_allocate_number(mp, &math->md_arc_tol_k, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_fraction_one_t, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_fraction_half_t, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_fraction_three_t, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_fraction_four_t, mp_fraction_type);
    /* |angles| */
    mp_double_allocate_number(mp, &math->md_three_sixty_deg_t, mp_angle_type);
    mp_double_allocate_number(mp, &math->md_one_eighty_deg_t, mp_angle_type);
    mp_double_allocate_number(mp, &math->md_negative_one_eighty_deg_t, mp_angle_type);
    /* various approximations */
    mp_double_allocate_number(mp, &math->md_one_k, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_sqrt_8_e_k, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_twelve_ln_2_k, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_coef_bound_k, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_coef_bound_minus_1, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_twelvebits_3, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_twentysixbits_sqrt2_t, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_twentyeightbits_d_t, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_twentysevenbits_sqrt2_d_t, mp_fraction_type);
    /* thresholds */
    mp_double_allocate_number(mp, &math->md_fraction_threshold_t, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_half_fraction_threshold_t, mp_fraction_type);
    mp_double_allocate_number(mp, &math->md_scaled_threshold_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_half_scaled_threshold_t, mp_scaled_type);
    mp_double_allocate_number(mp, &math->md_near_zero_angle_t, mp_angle_type);
    ...
    /* functions */
    math->md_from_int                 = mp_double_set_from_int;
    math->md_from_boolean             = mp_double_set_from_boolean;
    math->md_from_scaled              = mp_double_set_from_scaled;
    math->md_from_double              = mp_double_set_from_double;
    math->md_from_addition            = mp_double_set_from_addition;
    ...
    math->md_from_int_mul             = mp_double_set_from_int_mul;
    math->md_negate                   = mp_double_negate;
    math->md_add                      = mp_double_add;
    math->md_subtract                 = mp_double_subtract;
    math->md_half                     = mp_double_half;
    math->md_do_double                = mp_double_double;
    math->md_abs                      = mp_double_abs;
    math->md_clone                    = mp_double_clone;
    math->md_negated_clone            = mp_double_negated_clone;
    math->md_abs_clone                = mp_double_abs_clone;
    math->md_swap                     = mp_double_swap;
    math->md_add_scaled               = mp_double_add_scaled;
    math->md_multiply_int             = mp_double_multiply_int;
    math->md_divide_int               = mp_double_divide_int;
    math->md_to_int                   = mp_double_to_int;
    math->md_to_boolean               = mp_double_to_boolean;
    math->md_to_scaled                = mp_double_to_scaled;
    math->md_to_double                = mp_double_to_double;
    math->md_odd                      = mp_double_odd;
    math->md_equal                    = mp_double_equal;
    math->md_less                     = mp_double_less;
    math->md_greater                  = mp_double_greater;
    math->md_non_equal_abs            = mp_double_non_equal_abs;
    math->md_round_unscaled           = mp_double_unscaled;
    math->md_floor_scaled             = mp_double_floor;
    ...
    math->md_scaled_to_fraction       = mp_double_scaled_to_fraction;
    math->md_scaled_to_angle          = mp_double_scaled_to_angle;
    math->md_angle_to_scaled          = mp_double_angle_to_scaled;
    ...
    math->md_modulo                   = mp_double_modulo;
    ...
    /* housekeeping */
    math->md_free_math                = mp_double_free_math;
