...
introduced at the \CONTEXT\ 2025 meeting. The device is driven by a
\RASPBERRYPICO\ for which the distribution has software so you can assemble your
own (the first tests used a large silicon led strip).

Assuming a device 1, we have one (squid), four (quadrant) or eight (segment)
visualizers where each represents a run. By default context limits the number of
runs to nine so the last quadrant or segment is reused. In practice one never has
that many runs, as it indicates some form of oscillation.
...
In \LINUX\ a \RASPBERRYPICO\ will normally be \type {/dev/ttyACM0} but there we
also need to make sure that we get access. You need to be a member of the \type
