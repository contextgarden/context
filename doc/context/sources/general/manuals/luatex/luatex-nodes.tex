...
\NC getpre     \NC returns the \type {pre} head and tail fields \NC \NR
\NC getpost    \NC returns the \type {post} head and tail fields \NC \NR
\NC getreplace \NC returns the \type {replace} head and tail fields \NC \NR
...
These accessors can gain you some time when you do extensive processing of node
lists but only when you do many calls (a simple pass over the list using key
based access is normally pretty fast), which can happen when you need to look
into glyphs and discretionaries that make up most of the text stream.
...
\NC \type {getpre}               \NC \yes \NC \yes  \NC \NR
\NC \type {getpost}              \NC \yes \NC \yes  \NC \NR
\NC \type {getreplace}           \NC \yes \NC \yes  \NC \NR
