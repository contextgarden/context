...
It doesn't make sense to add getters for all fields, but we migth eventually do that
for consistency but we start with the most likely candidates. In complex documents,
many node and fields types never get seen, or seen only a few times, but for instance
glyphs are candidates for such optimization. The \type {node.direct} interface has
some more helpers. \footnote {We can define the helpers in the node namespace with
\type {getfield} which is about as efficient, so at some point we might provide
that as module.} The following table summarized this:
