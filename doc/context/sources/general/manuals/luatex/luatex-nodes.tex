...
\NC \type{attr}   \NC node   \NC list of attributes \NC \NR
\NC \type{stream} \NC number \NC \TEX's stream id number \NC \NR
\NC \type{data}   \NC string \NC a string representing the token list to be written \NC \NR
\NC \type{value}  \NC table  \NC a table representing the token list to be written \NC \NR
\LL
\stoptabulate
...
In \type {setfield} and \type {getfield} specials and writes accept \type {data}
and \type {value} as equivalent keys. In the case of a write whatsit getting the
\type {value} returns the token list as table, while \type {data} returns the
expansion as a string (version 1.22, 2026).
