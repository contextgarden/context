...
When \prm {ignoreprimitiveerror} is set to~1, the error \quotation {Infinite glue
shrinkage found in box being split} that occurs when splitting a box containing
infinite negative glue is turned into a warning.

% This is a kind of bad implementation: it could have been a bitset. The name
% primitive error has no relation to splitting. The source puts this primitive
% into the \ETEX\ category so it's a compatibility hack.
