...
There are more efficient helpers. Some are applied to a path, like in the example
below:
...
These mostly make sense in macros that manipulate paths, so for examples you can
check their usage in the \CONTEXT\ distribution. In addition there are also \typ
{firstdirection}, \typ {lastdirection}, \typ {firstunitdirection}, \typ
{lastunitdirection}
...
Again, these are mostly meant for extensive path manipulations. There are also
\typ {delta*} variants. Anyway, these two examples give us:

\startlinecorrection
    \startcombination[distance=1cm,align=middle]
        {\getbuffer[a]} {}
        {\getbuffer[b]} {}
    \stopcombination
\stoplinecorrection
...
\startsection[title=Segments]

Because we can construct a path in segments using \type {&&} we have methods for
accessing these segments. The following example demonstrates how we can access
these segments. We define a path that is made from two segments. We \type
{eofill} them, as we do for instance with glyphs in fonts. Here we have two
segments. The \type {segments} operators returns the number of segments, and
\type {segment} returns the subpath range. The \type {subpath} operator accepts a
numeric in which case it will return the segment.
...
\startMPcode
    ...
    draw textext("\tx we have " & decimal segments p & " segments");
\stopMPcode
\stopbuffer

\typebuffer[option=TEX]

In order to let the \type{eofill} do its work we have to close the path properly. The
next snippet has the same effect:
...
\startsection[title=Checking progress]

While constructing a path you have access to the coordinates of the
last added point:
...
We also have \type {previousx}, \type {previousy} and \type {previousxy} but be
aware of the fact that when you consult these after a first knot, these can have
a value related to a previous path. This is on purpose because we can use this to
pick up a path made from segments. When it has huge (infinite) values that
indicates a reset, for instance when we cycle. This might evolve a bit
as we proceed.
...
\startsection[title=Triplets]

In a way we can see a triplet as used in \RGB\ colors as a three dimensional
coordinate and in the future we might actually add some native support
for it. For the moment we provide \type {xpart}, \type {ypart},
\type {zpart} and \type {wpart} for the triplet and quadruplets.

A consequence is that some operations that one would not expect to work on
colors, now actually do. So, one can multiply them and for instance apply the
\type {normalize} unary operator.

\stopsection

\startsection[title=Nice to know]

It is hard to document every trick which is why users can peek in the source and
see what can be done. In \in {figure} [fig:roundpath] we see a possible
application for \type {round}.
...
The \type {round} command can be applied to various data types, which is true for more
such commands. If you miss something, just ask for it.

\startplacefigure[title={An a-typical application of \type {round}.},reference=fig:roundpath]
    \framed{\getbuffer}
\stopplacefigure

There are some helpers that only make sense in specific situations, take for
instance this one:
...
Although can use this for special effects, its's main application is in
situations where we use a path as a data structure, an efficient collection of
points.
...
\startMPcode
draw image (
    draw sortedpath fulloctagon xyscaled (5cm,1cm) ;
    draw sortedpath fullsquare  xyscaled (5cm,1cm) xshifted  6cm ;
    draw sortedpath fullhexagon xyscaled (5cm,1cm) xshifted 12cm ;
) withcolor "darkblue" withpen pencircle scaled 1mm ;
\stopMPcode
\stopbuffer
...
Some commands can be improved but best can be left untouched because they might
be used in specific situations. The next two cutters are for instance used in a
presentation style that already was present in \MKII.

\startbuffer
\startMPcode
def demo (expr p) =
    image (
        drawarrow           p scaled 20mm withcolor "darkred"   ;
...
As long as you keep in mind that in matters where the first point is, you're
probably fine. Also, there might in the meantime be better ways to get the result
you want.

\startlinecorrection \getbuffer \stoplinecorrection

Although you can use \type {cycle} to close a curve you might want
a different solution, like this:
...
Such helpers can also be looked at to get ideas of how to manipulate graphics
because one easily forget that they are around. If you look at the source you
will notice that they are just selectively applied \typ {.. cycle} and \typ {--
cycle} operations.

\startlinecorrection \getbuffer \stoplinecorrection

For some trickery normalization can be needed and that is what \typ
{xnormalized}, \typ {ynormalized} and \typ {xynormalized} do. Here is an example:
...
It also demonstrates the difference between these (predefined) \type {full} and
\type {unit} paths.

\startlinecorrection \getbuffer \stoplinecorrection

The version of \type {drawdot} that we have in \LUAMETAFUN\ also handles a path and
picture. It actually generates a rather efficient output.
...
Looking at the definition of this macro might actually teach you some tricks as it
used functionality not present in stock \METAPOST.

\startlinecorrection \getbuffer \stoplinecorrection

% Here is another curious one. We efficiently loop over a path and apply some
% macro to it and from the \quote {return} values we constrict a new path. The
% problem is of course that you get the control points true so
%
% \startbuffer
% \startMPcode
% vardef foo (expr what, p) =
%     draw p withpen pencircle scaled (1mm/what) ; p
% enddef ;
%
% draw processpath (fullcircle xyscaled (6cm,2cm)) (foo)
%     withpen pencircle scaled .25mm withcolor "darkred" ;
% \stopMPcode
% \stopbuffer
%
% \typebuffer[option=TEX]
%
% Of course just looping with \type {within} is often better but we didn't had that
% one yet.
%
% \startlinecorrection \getbuffer \stoplinecorrection

Although \METAPOST\ relates to \POSTSCRIPT\ its ancestor \METAFONT\ is slightly
different when it comes to transform matrices. The following definitions from \METAFUN\
show this:
...
The second definition can come in handy when you operate in a \POSTSCRIPT, \PDF, \SVG\ or
similar regime.
...
The term transform matrix is often use in this context and it also shows up when
fonts are involved. The first and third parameter are scales, the second and
third slant and/or rotate, while the last two move.
