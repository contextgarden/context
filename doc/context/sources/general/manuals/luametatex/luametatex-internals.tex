...
Here every token is fed into the interpreter. The \type {\starttext} is a macro
(control sequence) so it gets expanded and its stored tokens get interpreted. The
letters become (to be discussed) nodes in a linked list of content. In this case
the tokens are not stored and discarded as we read on.
...
An example is macros, like the mentioned \typ {\starttext} that refers to a token
list that starts the typesetting process. When reading the input from file these
commands and macros are looked up in a hash table. There are also built-in
commands that generate a hash entry. For instance when you define a counter or a
font, the given name becomes a hash entry that points to a memory location (again
an index).

Here it gets more complex. A hash table is used to lookup primitive commands like
\type {\hbox} and \type {\font} as well as \type {\starttext}. The string is
converted into an integer within a specific range. That integer is then an index
into a table like we saw before, with two halfwords per slot.
...
irrelevant on todays machines. So how about \LUAMETATEX ?
