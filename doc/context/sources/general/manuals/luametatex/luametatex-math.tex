...
\startsubsection[title={Script commands \prm {indexedsuperscript}, \prm {indexedsubscript}, \prm {indexedsuperprescript} and \prm {indexedsubprescript}}]

\topicindex {math+styles}
\topicindex {math+scripts}
\topicindex {math+indices}

Sometimes a script acts as an index. For various purposes we can therefore tag
super and subscripts as such. The rendering is the same. Often indexed scripts
are used with multi scripts.

\startbuffer[script]
$
    x\superscript       {1}\subscript       {2} =
    x\indexedsuperscript{1}\indexedsubscript{2} =
    x\superscript       {1}\indexedsubscript{2} =
    x\indexedsuperscript{1}\subscript       {2}
$
\stopbuffer
...
\NC \type {^^^}  \NC indexed \NC super \NC pre  \NC \NR
\NC \type {___}  \NC indexed \NC sub   \NC pre  \NC \NR
\NC \type {^^^^} \NC indexed \NC super \NC pre  \NC \NR
\NC \type {____} \NC indexed \NC sub   \NC pre  \NC \NR
\stoptabulate

The indexed variants force a script to be marked as index. In versions upto 2.10
...
\startsubsection[title={\prm {mathdiscretionary} and \prm {hmcode}}]
