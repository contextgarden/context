...
When Mikael and I were playing with \POTRACE\ and eventually ended up by turning
...
\METAFONT s are defined the results were mixed. Filled shaped could be made to
...
some memories came back but I couldn't find the files or code at that time. It
might relate to the fact that I always start from specifications. However, when
in January 2024 I had to fix some issue with embedding an old \TYPEONE\ font,
...
a font resource, most likely a \TYPEONE\ font. This is specified in a map file
that also permits specifying an encoding vector. For some reason it is not
possible to map to other font formats other than \TYPEONE\ and \TRUETYPE. No
...
bark about a missing font resource: it always wants a bitmap. Now, one can
...
lost in the \KPSE\ library that manages \PK\ fonts.
...
searched for the mandate scale and one or more glyphs. A simple test then becomes:
...
And then use a safe character present in most fonts at the same position in the
used encoding:

\starttyping
\starttext
    \font\test=cmr10mine \test \char 65
\stoptext
\stoptyping
...
\PK\ fonts being required in order for glyph containers to kick in, I suspect
that it's a mostly unknown feature, but one that actually can be made to work. It
could even work better (with respect to file lookup and font scaling) when it is
decoupled from the \PK\ requirements. But what worries me more is that after 25
years I hardly recognize the (reasonable) \PERL\ code and little manual although
it slowly came back. At that I time I could not have predicted where we are now
with respect to \TYPETHREE\ support, which in \LMTX\ is very much kicking and
alive. \footnote {At that time \TYPETHREE\ in the perspective of \TEX\ always had
a bad ring, probably because the many files that were around with too low
resolution bitmap fonts. The \PDF\ readers were not particularly good in
rendering them either. But with outlines this format can be fun!}
